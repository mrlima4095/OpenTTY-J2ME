...
public class Lua {
    ...
    public class LuaFunction implements CommandListener, ItemCommandListener, ItemStateListener {
        ...
        public Object internals(Vector args) throws Exception {
            ...
            // Package: os
            else if (MOD == EXEC) { if (args.isEmpty()) { } else { return new Double(midlet.processCommand(toLuaString(args.elementAt(0)), true, (args.size() < 2) ? id : ((args.elementAt(1) instanceof Boolean) ? ((Boolean) args.elementAt(1) ? 1000 : id) : gotbad(2, "execute", "boolean expected, got " + ((Integer) type(args.elementAt(1))))), pid, stdout, scope)); } }
