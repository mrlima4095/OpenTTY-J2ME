...
public class OpenTTY extends MIDlet implements CommandListener {
    ...
    private void processCommand(String command) {
        ...
        else if (mainCommand.equals("j2buster") || mainCommand.equals("jb")) { new J2Buster(argument); }
    ...
    public class J2Buster implements CommandListener { private String fullUrl, url; private String[] wordlist; private List pages; private Command openCommand, saveCommand, backCommand; public J2Buster(String args) { if (args == null || args.length() == 0) { return; } this.url = args; pages = new List("J2Buster (" + url + ")", List.IMPLICIT); wordlist = split(loadRMS("j2buster", 1), '\n'); if (wordlist == null || wordlist.length == 0) { wordlist = split(read("/java/etc/j2buster"), '\n'); } openCommand = new Command("Get Request", Command.OK, 1); saveCommand = new Command("Save Result", Command.OK, 1); backCommand = new Command("Back", Command.BACK, 1); pages.addCommand(openCommand); pages.addCommand(saveCommand); pages.addCommand(backCommand); pages.setCommandListener(this); new Thread(new Runnable() { public void run() { for (int i = 0; i < wordlist.length; i++) { String fullUrl = url.startsWith("http://") || url.startsWith("https://") ? url + "/" + wordlist[i] : "http://" + url + "/" + wordlist[i]; try { if (GoVerify(fullUrl)) { pages.append("/" + wordlist[i], null); } } catch (IOException e) { Alert alert = new Alert(null, e.getMessage(), null, AlertType.ERROR); alert.setTimeout(Alert.FOREVER); display.setCurrent(alert); } } } }).start(); display.setCurrent(pages); } private boolean GoVerify(String fullUrl) throws IOException { HttpConnection conn = null; InputStream is = null; try { conn = (HttpConnection) Connector.open(fullUrl); conn.setRequestMethod(HttpConnection.GET); int responseCode = conn.getResponseCode(); return (responseCode == HttpConnection.HTTP_OK); } finally { if (is != null) { is.close(); } if (conn != null) { conn.close(); } } } private String GoSave(List pages) { StringBuffer sb = new StringBuffer(); for (int i = 0; i < pages.size(); i++) { sb.append(pages.getString(i)); if (i < pages.size() - 1) { sb.append("\n"); } } return sb.toString(); } public void commandAction(Command c, Displayable d) { if (c == openCommand) { processCommand("wget " + url + pages.getString(pages.getSelectedIndex())); } else if (c == saveCommand) { nanoContent = GoSave(pages); } else if (c == backCommand) { processCommand("xterm"); } } }
