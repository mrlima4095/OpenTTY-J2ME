...
public class OpenTTY extends MIDlet implements CommandListener {
    ...
    public int processCommand(String command, boolean ignore, boolean root) { 
        ...
        else if (mainCommand.equals("stty")) { if (argument.equals("")) { } else { MAX_STDOUT_LEN = getNumber(argument, MAX_STDOUT_LEN, true); } }
        // |
        ...
        // |
        // General Utilities
        else if (mainCommand.equals("history")) { new Monitor("history", root); }
    ...
    private int kernel(String command, boolean root) {
        ...
        if (mainCommand.equals("") || mainCommand.equals("monitor") || mainCommand.equals("process")) { new Monitor(argument, root); } 
...
/* Lua.java
   Mini Lua interpreter for J2ME (single-file, no AST heavy)
   Usage in OpenTTY: new Lua(this, root).run(code);
*/
...
public class Lua {
    ...
    // Entry point: run code (script)
    public void run(String code) {
        if (code == null) return;
        try {
            Interpreter interp = new Interpreter(code);
            interp.execBlock();
        } catch (InterpreterError e) {
            midlet.processCommand("echo lua: error: " + e.getMessage());
        }
    }
    ...
    private final class Interpreter {
        ...
        private int pos;
        private Token cur;
        private Hashtable functions; // name -> Function
        private Vector envStack; // stack of Hashtables for variables; index 0 = globals

        Interpreter(String s) {
            this.src = s;
            this.pos = 0;
            this.functions = new Hashtable();
            this.envStack = new Vector();
            this.envStack.addElement(new Hashtable()); // globals
            nextToken();
            registerBuiltins();
        }

        private void registerBuiltins() {
            // register print and exec
            Function printF = new Function(new String[] {"text"}, null) {
                Object call(Vector args) {
                    Object v = args.size() > 0 ? args.elementAt(0) : null;
                    String out = tostring(v);
                    midlet.processCommand("echo " + out);
                    return null;
                }
            };
            functions.put("print", printF);

            Function execF = new Function(new String[] {"cmd"}, null) {
                Object call(Vector args) {
                    String cmd = args.size() > 0 ? tostring(args.elementAt(0)) : "";
                    midlet.processCommand(cmd);
                    return null;
                }
            };
            functions.put("exec", execF);
        }

        // High level: execute a block until EOF (or until returning to caller)
        void execBlock() {
        ...
        private void execStatement() {
            if (cur.type == Token.KEYWORD && cur.text.equals("function")) {
                parseFunctionDef();
                return;
            }
            if (cur.type == Token.KEYWORD && cur.text.equals("if")) {
                parseIf();
                return;
            }
            if (cur.type == Token.KEYWORD && cur.text.equals("while")) {
                parseWhile();
                return;
            }
            if (cur.type == Token.KEYWORD && cur.text.equals("return")) {
                nextToken();
                Object val = parseExpression();
                // bubble up a ReturnException to caller
                throw new ReturnException(val);
            }
            // assignment or expression
            if (cur.type == Token.IDENT) {
                // could be var = expr  OR funcCall(...)
                Token look = peekToken();
                if (look.type == Token.SYMBOL && look.text.equals("=")) {
                    // assignment
            ...
            } else if (cur.type == Token.SYMBOL && cur.text.equals(";")) {
                nextToken();
                return;
            } else {
                // expression / skip
        ...
        // Function definition: function name(arg1, arg2) ... end
        private void parseFunctionDef() {
            nextToken(); // skip 'function'
            if (cur.type != Token.IDENT) throw new InterpreterError("expected function name");
            String fname = cur.text;
            nextToken();
            expect("(");
            Vector paramsV = new Vector();
            if (!(cur.type == Token.SYMBOL && cur.text.equals(")"))) {
                while (true) {
                    if (cur.type != Token.IDENT) throw new InterpreterError("expected param name");
                    paramsV.addElement(cur.text);
                    nextToken();
                    if (cur.type == Token.SYMBOL && cur.text.equals(",")) {
                        nextToken();
                        continue;
            ...
            int bodyStart = pos;
            // we need to find the matching 'end' token respecting nested function/if/while
            int depth = 0;
            int savePos = pos;
            Token tmp = cur;
            while (true) {
                if (tmp.type == Token.EOF) throw new InterpreterError("unterminated function");
                if (tmp.type == Token.KEYWORD) {
                    if (tmp.text.equals("function") || tmp.text.equals("if") || tmp.text.equals("while")) depth++;
                    else if (tmp.text.equals("end")) {
                        if (depth == 0) break;
                        depth--;
                    }
                }
                tmp = nextTokenLocal(tmp, savePos); // move
                savePos = pos;
            }
            // body substring from current pos to position of 'end' token start
            int bodyEndPos = tokenStartPos(savePos);
            String body = src.substring(bodyStart, bodyEndPos);
            // now advance tokens until after 'end'
            // we already located 'end' at tmp; set pos appropriately by re-parsing from current 'cur'
            // for simplicity: parse tokens forward until we consume the 'end' keyword
            while (!(cur.type == Token.KEYWORD && cur.text.equals("end"))) {
                nextToken();
                if (cur.type == Token.EOF) throw new InterpreterError("unterminated function");
            }
            nextToken(); // consume 'end'

            // build function object
            String[] params = new String[paramsV.size()];
            for (int i = 0; i < paramsV.size(); i++) params[i] = (String) paramsV.elementAt(i);
            Function f = new Function(params, body);
            functions.put(fname, f);
        }

        // If statement: if expr then ... [else ...] end
        private void parseIf() {
            ...
            if (truth) {
                // execute then-block
                execSubBlockUntil(new String[] {"else", "end"});
                // skip to after block (we expect else or end)
                if (cur.type == Token.KEYWORD && cur.text.equals("else")) {
                    // skip else block
                    // consume until end
                    skipBlockUntil("end");
                    expect("end");
                    nextToken();
                } else if (cur.type == Token.KEYWORD && cur.text.equals("end")) {
                    nextToken();
                }
            } else {
                // skip then block
                skipBlockUntil("else", "end");
                if (cur.type == Token.KEYWORD && cur.text.equals("else")) {
                    nextToken(); // after else
                    // execute else-block
                    execSubBlockUntil(new String[] {"end"});
                    expect("end");
                    nextToken();
                } else if (cur.type == Token.KEYWORD && cur.text.equals("end")) {
                    nextToken();
        ...
        private void parseWhile() {
            ...
            while (true) {
                ...
                if (!truth) {
                    ...
                    // after body, current should be 'end'
                    expect("end");
                    nextToken();
                    // go back: re-evaluate condition (we must reparse the condition)
                    // to reparse, create a new Interpreter over the condition source substring
                    // Simplify: we reset pos to condStartPos and re-tokenize from there within this interpreter.
                    pos = condStartPos;
                    nextToken();
                    continue;
                }
        ...
        // Execute a sub-block until a keyword in terminators is reached
        private void execSubBlockUntil(String[] terminators) {
            while (true) {
                if (cur.type == Token.EOF) throw new InterpreterError("unexpected EOF");
                if (cur.type == Token.KEYWORD) {
                    for (int i = 0; i < terminators.length; i++) {
                        if (cur.text.equals(terminators[i])) return;
                    }
                }
                execStatement();
            }
        ...
        // Skip tokens until encountering one of the keywords
        private void skipBlockUntil(String t1, String t2) {
            while (true) {
                if (cur.type == Token.EOF) return;
                if (cur.type == Token.KEYWORD) {
                    if (cur.text.equals(t1) || cur.text.equals(t2)) return;
                }
                nextToken();
            }
        }

        private void skipBlockUntil(String t) {
            while (true) {
                if (cur.type == Token.EOF) return;
                if (cur.type == Token.KEYWORD && cur.text.equals(t)) return;
                nextToken();
            }
        }
        ...
        private Object parseOr() {
            ...
            while (cur.type == Token.KEYWORD && cur.text.equals("or")) {
                nextToken();
                Object right = parseAnd();
                if (isTruthy(left) || isTruthy(right)) left = Boolean.TRUE;
                else left = Boolean.FALSE;
            }
            return left;
        }
        private Object parseAnd() {
            Object left = parseNot();
            while (cur.type == Token.KEYWORD && cur.text.equals("and")) {
                nextToken();
        ...
        private Object parseNot() {
            if (cur.type == Token.KEYWORD && cur.text.equals("not")) {
                nextToken();
                Object v = parseComparison();
                return Boolean.valueOf(!isTruthy(v));
            }
            return parseComparison();
        }
        private Object parseComparison() {
            Object left = parseAddSub();
            while (cur.type == Token.SYMBOL && (cur.text.equals("==") || cur.text.equals("~=") || cur.text.equals("<") || cur.text.equals(">") || cur.text.equals("<=") || cur.text.equals(">="))) {
                String op = cur.text;
                nextToken();
        ...
        private Object parseAddSub() {
            ...
            while (cur.type == Token.SYMBOL && (cur.text.equals("+") || cur.text.equals("-"))) {
                String op = cur.text;
                nextToken();
        ...
        private Object parseMulDiv() {
            ...
            while (cur.type == Token.SYMBOL && (cur.text.equals("*") || cur.text.equals("/") || cur.text.equals("%"))) {
                String op = cur.text;
                nextToken();
        ...
        private Object parseUnary() {
            if (cur.type == Token.SYMBOL && cur.text.equals("-")) {
                ...
                throw new InterpreterError("invalid operand for unary -");
            }
            return parsePrimary();
        }
        private Object parsePrimary() {
            if (cur.type == Token.NUMBER) {
                Double d = Double.valueOf(cur.text);
                nextToken();
                return d;
            }
            ...
            if (cur.type == Token.KEYWORD) {
                if (cur.text.equals("true")) { nextToken(); return Boolean.TRUE; }
                if (cur.text.equals("false")) { nextToken(); return Boolean.FALSE; }
                if (cur.text.equals("nil")) { nextToken(); return null; }
            }
            if (cur.type == Token.IDENT) {
                String name = cur.text;
                Token look = peekToken();
                if (look.type == Token.SYMBOL && look.text.equals("(")) {
                    ...
                    expect(")");
                    // call function
                    Object res = callFunction(name, args);
                    return res;
                } else {
                    // variable lookup
                    nextToken();
            ...
            throw new InterpreterError("unexpected token: " + cur.text);
        }

        // Utilities: arithmetic / compare
        private Object arithmetic(String op, Object a, Object b) {
            Double A = (a instanceof Double) ? (Double) a : null;
            Double B = (b instanceof Double) ? (Double) b : null;
            if (A == null || B == null) throw new InterpreterError("arithmetic on non-number");
            double av = A.doubleValue(), bv = B.doubleValue();
            if (op.equals("+")) return new Double(av + bv);
            if (op.equals("-")) return new Double(av - bv);
            if (op.equals("*")) return new Double(av * bv);
            if (op.equals("/")) return new Double(av / bv);
            if (op.equals("%")) return new Double(av % bv);
            throw new InterpreterError("unknown op " + op);
        }
        private Object compare(String op, Object a, Object b) {
            // if both numbers
            if (a instanceof Double && b instanceof Double) {
                double av = ((Double) a).doubleValue(), bv = ((Double) b).doubleValue();
                if (op.equals("==")) return Boolean.valueOf(av == bv);
                if (op.equals("~=")) return Boolean.valueOf(av != bv);
                if (op.equals("<")) return Boolean.valueOf(av < bv);
                if (op.equals(">")) return Boolean.valueOf(av > bv);
                if (op.equals("<=")) return Boolean.valueOf(av <= bv);
                ...
                // fallback to string compare / nulls
                String sa = tostring(a), sb = tostring(b);
                if (op.equals("==")) return Boolean.valueOf(sa.equals(sb));
                if (op.equals("~=")) return Boolean.valueOf(!sa.equals(sb));
                if (op.equals("<")) return Boolean.valueOf(sa.compareTo(sb) < 0);
                if (op.equals(">")) return Boolean.valueOf(sa.compareTo(sb) > 0);
                if (op.equals("<=")) return Boolean.valueOf(sa.compareTo(sb) <= 0);
                if (op.equals(">=")) return Boolean.valueOf(sa.compareTo(sb) >= 0);
            }
        ...
        // Function call
        private Object callFunction(String name, Vector args) {
            Object obj = functions.get(name);
            if (obj == null) throw new InterpreterError("function '" + name + "' not found");
            Function f = (Function) obj;
            return f.exec(args);
        }

        // Variable helpers (search envStack from top to bottom)
        private Object getVar(String name) {
            for (int i = envStack.size() - 1; i >= 0; i--) {
                Hashtable h = (Hashtable) envStack.elementAt(i);
                if (h.containsKey(name)) return h.get(name);
            }
            return null;
        }
        private void setVar(String name, Object val) {
            // assign to nearest scope that already has it, otherwise to globals
            for (int i = envStack.size() - 1; i >= 0; i--) {
                Hashtable h = (Hashtable) envStack.elementAt(i);
                if (h.containsKey(name)) { h.put(name, val); return; }
            }
            // global
            ((Hashtable) envStack.elementAt(0)).put(name, val);
        }

        // Execute function body: create new local scope with params
        private abstract class Function {
            String[] params;
            String body; // null for builtins
            Function(String[] params, String body) { this.params = params; this.body = body; }
            Object exec(Vector args) {
                if (this.body == null) return call(args); // builtin override
                // create a parser dedicated to the body
                Interpreter sub = new Interpreter(this.body);
                // share functions and global env
                sub.functions = Interpreter.this.functions;
                sub.envStack = new Vector();
                // global env shares same global table
                sub.envStack.addElement(Interpreter.this.envStack.elementAt(0));
                // local scope
                Hashtable locals = new Hashtable();
                for (int i = 0; i < params.length; i++) {
                    Object v = i < args.size() ? args.elementAt(i) : null;
                    locals.put(params[i], v);
                }
                sub.envStack.addElement(locals);
                try {
                    sub.execBlock();
                } catch (ReturnException re) {
                    return re.value;
            ...
            // for builtins override call(Vector)
            Object call(Vector args) { return null; }
        }

        // Tokenizer + helpers
        private Token nextTokenLocal(Token ref, int savedPos) {
            // move forward by reading token at current global pos
            int oldPos = pos;
            Token oldCur = cur;
            int start = pos;
            Token t = readNextToken(); // advances pos
            // restore to previous state so only token is returned as tmp
            // BUT we need to return t while restoring actual pos & cur to previous (so not to disturb main stream)
            int curPos = pos;
            pos = start; // revert
            cur = oldCur;
            return t;
        }
        private int tokenStartPos(int p) {
            return p;
        }

        private void expect(String s) {
            if (!(cur.type == Token.SYMBOL && cur.text.equals(s) || (cur.type == Token.KEYWORD && cur.text.equals(s)))) {
                throw new InterpreterError("expected '" + s + "', found '" + cur.text + "'");
            }
            nextToken();
        }
        ...
        private boolean isTruthy(Object v) {
            ...
            if (v instanceof Double) return ((Double) v).doubleValue() != 0.0;
        ...
        private Token peekToken() {
            // naive: save pos and cur, read one token, restore
            int savedPos = pos;
            Token savedCur = cur;
        ...
        private Token readNextToken() {
            skipSpaces();
            if (pos >= src.length()) return new Token(Token.EOF, "");
            ...
            // number
            if ((c >= '0' && c <= '9') || (c == '.' && pos + 1 < src.length() && Character.isDigit(src.charAt(pos + 1)))) {
                int start = pos;
                boolean dot = false;
                while (pos < src.length()) {
                    ...
                    if (cc >= '0' && cc <= '9') { pos++; continue; }
                    break;
                }
                return new Token(Token.NUMBER, src.substring(start, pos));
            }
            // string "
            if (c == '"') {
                pos++;
                ...
                throw new InterpreterError("unterminated string");
            }
            // identifier / keywords
            if (isIdentifierStart(c)) {
                int start = pos;
                pos++;
                while (pos < src.length() && isIdentifierPart(src.charAt(pos))) pos++;
                String word = src.substring(start, pos);
                if (isKeyword(word)) return new Token(Token.KEYWORD, word);
            ...
            if (pos + 1 < src.length()) {
                ...
                if (two.equals("==") || two.equals("<=") || two.equals(">=") || two.equals("~=")) {
                    pos += 2;
                    return new Token(Token.SYMBOL, two);
                }
            }
            // single-char symbols
            pos++;
            return new Token(Token.SYMBOL, String.valueOf(c));
        }
        ...
        private boolean isKeyword(String w) {
            String[] kw = new String[] {"and","break","do","else","elseif","end","false","for","function","if","in","local","nil","not","or","repeat","return","then","true","until","while","do"};
            for (int i = 0; i < kw.length; i++) if (kw[i].equals(w)) return true;
            return false;
        }
    }
    ...
    private static final class Token {
        static final int EOF = 0, NUMBER = 1, STRING = 2, IDENT = 3, SYMBOL = 4, KEYWORD = 5;
        int type;
        String text;
        Token(int t, String s) { this.type = t; this.text = s; }
    }

    // Exceptions & errors
    private static final class InterpreterError extends RuntimeException {
        InterpreterError(String m) { super(m); }
    }
    private static final class ReturnException extends RuntimeException {
        Object value;
