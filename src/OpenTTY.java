...
public class OpenTTY extends MIDlet implements CommandListener {
    ...
    private String logs = "", path = "/home/", build = "2025-1.16-02x21"; 
    ...
    private int processCommand(String command, boolean ignore, boolean root) { 
        ...
        else if (mainCommand.equals("su")) { if (root) { if (username.equals("root")) { username = loadRMS("OpenRMS"); } else { username = "root"; } } else { echoCommand("Permission denied!"); return 13; } }
        else if (mainCommand.equals("passwd")) { if (root) { if (argument.equals("")) { } else { writeRMS("/home/.passwd", "" + argument.hashCode()); } } else { echoCommand("Permission denied!"); return 13; } }
        else if (mainCommand.equals("logout")) { if (loadRMS("OpenRMS").equals(username)) { if (root) { writeRMS("/home/OpenRMS", ""); processCommand("exit", ignore, root); } else { echoCommand("Permission denied!"); return 13; } } else { username = loadRMS("OpenRMS"); } }
        ...
        // |
        // Chain executors
        else if (mainCommand.equals("exec")) { String[] CMDS = split(argument, '&'); for (int i = 0; i < CMDS.length; i++) { processCommand(CMDS[i].trim(), ignore, root); } }
        else if (mainCommand.equals("execute")) { String[] CMDS = split(argument, ';'); for (int i = 0; i < CMDS.length; i++) { processCommand(CMDS[i].trim(), ignore, root); } }
        ...
        else if (mainCommand.equals("mkdir")) { if (argument.equals("")) { } else { argument = argument.endsWith("/") ? argument : argument + "/"; argument = argument.startsWith("/") ? argument : path + argument; if (argument.startsWith("/mnt/")) { try { FileConnection CONN = (FileConnection) Connector.open("file:///" + argument.substring(5), Connector.READ_WRITE); if (!CONN.exists()) { CONN.mkdir(); CONN.close(); } else { echoCommand("mkdir: " + basename(argument) + ": found"); } CONN.close(); } catch (SecurityException e) { echoCommand(getCatch(e)); return 13; } catch (IOException e) { echoCommand(getCatch(e)); return 1; } } else if (argument.startsWith("/home/")) { echoCommand("mkdir: 405 Method not allowed"); return 3; } else if (argument.startsWith("/")) { echoCommand("read-only storage"); return 5; } } }
        ...
        else if (mainCommand.equals("@alert")) { try { display.vibrate(argument.equals("") ? 500 : Integer.parseInt(argument) * 100); } catch (NumberFormatException e) { echoCommand(getCatch(e)); return 2; } }
        ...
        else if (mainCommand.equals("return")) { try { return Integer.valueOf(argument); } catch (Exception e) { echoCommand(getCatch(e)); return 128; } }
    ...
    // |
    // Operators
    private int ifCommand(String argument, boolean ignore, boolean root) { 
        ...
        if (PARTS.length == 3) { 
            ...
            Double N1 = getNumber(PARTS[0]), N2 = getNumber(PARTS[2]); 

            if (N1 != null && N2 != null) { 
                if (PARTS[1].equals("==")) { CONDITION = N1.doubleValue() == N2.doubleValue(); } 
                else if (PARTS[1].equals("!=")) { CONDITION = N1.doubleValue() != N2.doubleValue(); } 
                else if (PARTS[1].equals(">")) { CONDITION = N1.doubleValue() > N2.doubleValue(); } 
                else if (PARTS[1].equals("<")) { CONDITION = N1.doubleValue() < N2.doubleValue(); } 
                else if (PARTS[1].equals(">=")) { CONDITION = N1.doubleValue() >= N2.doubleValue(); } 
                else if (PARTS[1].equals("<=")) { CONDITION = N1.doubleValue() <= N2.doubleValue(); } 
            } else { 
                if (PARTS[1].equals("startswith")) { CONDITION = PARTS[0].startsWith(PARTS[2]); } 
                else if (PARTS[1].equals("endswith")) { CONDITION = PARTS[0].endsWith(PARTS[2]); } 
                else if (PARTS[1].equals("contains")) { CONDITION = PARTS[0].indexOf(PARTS[2]) != -1; } 
                else if (PARTS[1].equals("==")) { CONDITION = PARTS[0].equals(PARTS[2]); } 
                else if (PARTS[1].equals("!=")) { CONDITION = !PARTS[0].equals(PARTS[2]); } 
            } 

            if (CONDITION != NEGATED) { return processCommand(CMD, ignore, root); } 
        } else if (PARTS.length == 2) { 
            if (PARTS[0].equals(PARTS[1])) { return processCommand(CMD, ignore, root); } 
        } else if (PARTS.length == 1) { 
            if (!PARTS[0].equals("")) { return processCommand(CMD, ignore, root); } 
        } 

        return 0; 
    }
    private int forCommand(String argument, boolean ignore, boolean root) { 
        argument = argument.trim(); 
        int firstParenthesis = argument.indexOf('('), lastParenthesis = argument.indexOf(')'); 
        ...
        if (KEY.startsWith("(")) { return 2; } if (KEY.startsWith("$")) { KEY = replace(KEY, "$", ""); } 

        String[] LINES = split(FILE, '\n'); 
        for (int i = 0; i < LINES.length; i++) { 
            if (LINES[i] != null || LINES[i].length() != 0) { 
                processCommand("set " + KEY + "=" + LINES[i], false, root); 
                int STATUS = processCommand(CMD, ignore, root); 
                processCommand("unset " + KEY, false, root); 
    ...
    private int caseCommand(String argument, boolean ignore, boolean root) { 
        argument = argument.trim(); 
        int firstParenthesis = argument.indexOf('('), lastParenthesis = argument.indexOf(')'); 
        if (firstParenthesis == -1 || lastParenthesis == -1 || firstParenthesis > lastParenthesis) { return 2; } 
        ...
        boolean CONDITION = false; boolean NEGATED = METHOD.startsWith("!"); if (NEGATED) { METHOD = METHOD.substring(1); } 
    ...
    private int importScript(String script, boolean root) {
        ...
        if (PKG.containsKey("mod") && PKG.containsKey("process.name")) { final String PROCESS = (String) PKG.get("process.name"); final String MOD = (String) PKG.get("mod"); new Thread("MIDlet-Mod") { public void run() { while (trace.containsKey(PROCESS)) { int STATUS = processCommand(MOD, true, root); if (STATUS != 0) { return STATUS; } } } }.start(); }
        // |
