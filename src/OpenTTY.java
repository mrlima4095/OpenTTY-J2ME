...
public class OpenTTY extends MIDlet implements CommandListener {
    ...
    public void startApp() {
        if (!app == true) {
            ...
            attributes.put("PATCH", "The Revolution"); attributes.put("VERSION", getAppProperty("MIDlet-Version")); attributes.put("RELEASE", "mod"); attributes.put("XVERSION", "0.5");
    ...
    private void processCommand(String command) {
        ...
        else if (mainCommand.equals("wget")) { if (argument.equals("")) { return; } else { nanoContent = request(argument.startsWith("http://") && argument.startsWith("https://") ? argument : "http://" + argument); } }
    ...
    private void lockCommand() { final Form lock = new Form(form.getTitle() + " - Locked"); final TextField userField = new TextField("Username", "", 256, TextField.ANY); lock.append(userField); lock.addCommand(new Command("Unlock", Command.OK, 1)); lock.addCommand(new Command("Exit", Command.EXIT, 2)); lock.setCommandListener(new CommandListener() { public void commandAction(Command c, Displayable d) { if (c.getCommandType() == Command.OK) { if (userField.getString().equals(username)) { display.setCurrent(form); } else if (!userField.getString().equals("")) { Alert alert = new Alert(form.getTitle(), "Wrong username!", null, AlertType.WARNING); alert.setTimeout(Alert.FOREVER); display.setCurrent(alert, lock); } } else if (command.getCommandType() == Command.EXIT) { processCommand("exit"); } } } ); display.setCurrent(lock); }
