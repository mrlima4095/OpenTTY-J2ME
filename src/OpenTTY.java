...
public class OpenTTY extends MIDlet implements CommandListener {
    ...
    private int processCommand(String command, boolean ignore, boolean root) { 
        ...
        else if (mainCommand.equals("bruteforce")) { String PID = genpid(); start("bruteforce", null, null, root); while (trace.containsKey(PID)) { int STATUS = processCommand(argument, ignore, root); if (STATUS != 0) { stop("bruteforce", root); return STATUS; } } }
    ...
    public class ItemLoader implements ItemCommandListener { 
        ...
        public void commandAction(Command c, Item item) { 
            if (c == run) { processCommand("xterm"); processCommand((String) PKG.get(node + ".cmd")); } 
    ...
    private int caseCommand(String argument, boolean ignore, boolean root) { 
        ...
        else if (METHOD.equals("trace")) { CONDITION = getprocess(EXPR) != null ? true : false; } 
    ...
    private int stop(String app, boolean root) {
        ...
        for (Enumeration KEYS = trace.keys(); KEYS.hasMoreElements();) {
            ...
            if (app.equals(name)) { STATUS = kill(KEY, false, root); if (STATUS != 0) { break; } }
        }
        return STATUS;
    }
