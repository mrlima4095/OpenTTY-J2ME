...
public class OpenTTY extends MIDlet implements CommandListener {
    ...
    // |
    // HTTP Interfaces
    private void pingCommand(String url) { if (url == null || url.length() == 0) { return; } if (!url.startsWith("http://") && !url.startsWith("https://")) { url = "http://" + url; } long start = System.currentTimeMillis(); try { HttpConnection conn = (HttpConnection) Connector.open(url); conn.setRequestMethod(HttpConnection.GET); int responseCode = conn.getResponseCode(); long end = System.currentTimeMillis(); echoCommand("Ping to " + url + " successful, time=" + (endTime - startTime) + "ms"); conn.close(); } catch (IOException e) { echoCommand("Ping to " + url + " failed: " + e.getMessage()); } }
    private void pongCommand(String address) { 
        if (address != null || address.length() == 0) { } 
            ...
            try {
                socket = (SocketConnection) Connector.open("socket://" + address);
                long endTime = System.currentTimeMillis();
                echoCommand("Pong to " + address + " successful, time=" + (endTime - startTime) + "ms");
            } catch (IOException e) { echoCommand("Pong to " + ip + ":" + port + " failed: " + e.getMessage()); }
        }
    }
