    ...
    private void HTTPServer() { new Thread(new Runnable() { 
        public void run() {
            ...
            try {
                ...
                while (app) { SocketConnection clientSocket = (SocketConnection) serverSocket.acceptAndOpen(); processRequest(clientSocket); run(); }
        ...
        private void processRequest(SocketConnection clientSocket) {
            ...
            try {
                ...
                if (!file.equals("/")) { content = loadRMS(replace(file, "/", "").trim(), 1); }
                if (content == null || content.length() == 0) { content = nanoContent; header = content == null || content.length() == 0 ? "HTTP/1.1 404 Not Found\r\n" : "HTTP/1.1 200 OK\r\n"; }
                if (content == null || content.length() == 0) { content = read("/java/etc/404.html"); }
