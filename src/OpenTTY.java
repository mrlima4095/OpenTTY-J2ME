...
public class OpenTTY extends MIDlet implements CommandListener {
    ...
    private void processCommand(String command, boolean ignore) {
        ...
        // |
        // Permissions
        else if (mainCommand.equals("chmod")) { if (argument.equals("")) { } else { Hashtable nodes = parseProperties("http=javax.microedition.io.Connector.http\nsocket=javax.microedition.io.Connector.socket\nfile=javax.microedition.io.Connector.file\nprg=javax.microedition.io.PushRegistry"); int status = 1; if (nodes.containsKey(argument)) { try { if (node.equals("http")) { ((HttpConnection) Connector.open("http://google.com")).close(); } else if (node.equals("socket")) { ((SocketConnection) Connector.open(env("socket://$REPO"))).close(); } else if (node.equals("file")) { FileSystemRegistry.listRoots(); } else if (node.equals("prg")) { PushRegistry.registerAlarm(getClass().getName(), System.currentTimeMillis() + 1000); } } catch (SecurityException e) { status = 2; } catch (Exception e) { status = 3; } } else if (argument.equals("*")) { Enumeration keys = nodes.keys(); while (keys.hasMoreElements()) { processCommand("chmod " + (String) keys.nextElement(), false); status = 4; } } else { echoCommand("chmod: " + node + ": not found"); status = 5; } if (status == 1) { MIDletLogs("add info Permission '" + (String) nodes.get(node) + "' granted"); } else if (status == 2) { MIDletLogs("add error Permission '" + (String) nodes.get(node) + "' denied"); } else if (status == 3) { MIDletLogs("add warn Unsupported API '" + (String) nodes.get(node) + "'"); } } }
        // |
        // General Utilities
