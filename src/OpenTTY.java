...
public class OpenTTY extends MIDlet implements CommandListener {
    ...
    private String logs = "", path = "/home/", build = "2025-1.16-02x27"; 
    ...
    private int processCommand(String command, boolean ignore, boolean root) { 
        ...
        else if (mainCommand.equals("logout")) { if (loadRMS("OpenRMS").equals(username)) { if (root) { writeRMS("/home/OpenRMS", ""); processCommand("exit", false, root); } else { echoCommand("logout: permission denied"); return 13; } } else { username = loadRMS("OpenRMS"); processCommand("sh", false); } }
    ...
    private String read(String filename) { try { if (filename.startsWith("/mnt/")) { FileConnection fileConn = (FileConnection) Connector.open("file:///" + filename.substring(5), Connector.READ); InputStream is = fileConn.openInputStream(); StringBuffer content = new StringBuffer(); int ch; while ((ch = is.read()) != -1) { content.append((char) ch); } is.close(); fileConn.close(); return env(content.toString()); } else if (filename.startsWith("/home/")) { String content = ""; try { RMS = RecordStore.openRecordStore(filename.substring(6), true); if (RMS.getNumRecords() >= 1) { byte[] data = RMS.getRecord(1); if (data != null) { content = new String(data); } } } catch (RecordStoreException e) { content = ""; } finally { if (RMS != null) { try { RMS.closeRecordStore(); } catch (RecordStoreException e) { } } } return content; } else { StringBuffer content = new StringBuffer(); InputStream is = getClass().getResourceAsStream(filename); if (is == null) { return ""; } InputStreamReader isr = new InputStreamReader(is, "UTF-8"); int ch; while ((ch = isr.read()) != -1) { content.append((char) ch); } isr.close(); return env(content.toString()); } } catch (IOException e) { return ""; } }
    ...
    public class Credentials implements CommandListener { 
        ...
        public Credentials(String args) { 
            if (args == null || args.length() == 0 || args.equals("login")) { 
                ...
                if (asking_user) { asking_user = true; screen.append(USER); } 
                if (asking_passwd) { screen.append(PASSWD); } 
            } 
            else { 
                TYPE = REQUEST; 
...
private String passwd(boolean write, String value) {
    ...
    private int writeRMS(String filename, byte[] data) { if (filename == null || filename.length() == 0) { return 2; } else if (filename.startsWith("/mnt/")) { try { FileConnection CONN = (FileConnection) Connector.open("file:///" + filename.substring(5), Connector.READ_WRITE); if (!CONN.exists()) { CONN.create(); } OutputStream OUT = CONN.openOutputStream(); OUT.write(data); OUT.flush(); } catch (SecurityException e) { echoCommand(getCatch(e)); return 13; } catch (Exception e) { echoCommand(getCatch(e)); return 1; } } else if (filename.startsWith("/home/")) { filename = filename.substring(6); try { RMS = RecordStore.openRecordStore(filename, true); if (RMS.getNumRecords() > 0) { RMS.setRecord(1, data, 0, data.length); } else { RMS.addRecord(data, 0, data.length); } } catch (RecordStoreException e) { } finally { if (RMS != null) { try { RMS.closeRecordStore(); } catch (RecordStoreException e) { } } } } else if (filename.startsWith("/")) { echoCommand("read-only storage"); return 5; } else { return writeRMS(path + filename, data); } return 0; }
    ...
    private void add2History(String command) { if (command.equals("") || command.equals("!!") || command.startsWith("#") || command.equals(getLastHistory())) { } else { history.addElement(command.trim()); } }
