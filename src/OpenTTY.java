...
public class OpenTTY extends MIDlet implements CommandListener {
    ...
    private String logs = "", path = "/home/", build = "2025-1.16-02x32"; 
    ...
    private int processCommand(String command, boolean ignore, boolean root) { 
        ...
        else if (mainCommand.equals("env")) { if (argument.equals("")) { Enumeration KEYS = attributes.keys(); while (KEYS.hasMoreElements()) { String KEY = (String) KEYS.nextElement(), VALUE = (String) attributes.get(KEY); if (!KEY.equals("OUTPUT") && !VALUE.equals("")) { echoCommand(KEY + "=" + VALUE.trim()); } } } else if (attributes.containsKey(argument)) { echoCommand(argument + "=" + (String) attributes.get(argument)); } else { echoCommand("env: " + argument + ": not found"); return 127; } }
        ...
        // |
        // PushRegistry
        else if (mainCommand.equals("prg")) { if (argument.equals("")) { argument = "5"; } try { PushRegistry.registerAlarm(getArgument(argument).equals("") ? "OpenTTY" : getArgument(argument), System.currentTimeMillis() + Integer.parseInt(getCommand(argument)) * 1000); } catch (NumberFormatException e) { echoCommand(getCatch(e)); return 2; } catch (ClassNotFoundException e) { echoCommand("prg: " + getArgument(argument) + ": not found"); return 127; } catch (Exception e) { echoCommand(getCatch(e)); return 3; } }
        ...
        else if (mainCommand.equals("cd")) { if (argument.equals("")) { path = "/home/"; } else if (argument.equals("..")) { if (path.equals("/")) { return 0; } int lastSlashIndex = path.lastIndexOf('/', path.endsWith("/") ? path.length() - 2 : path.length() - 1); path = (lastSlashIndex <= 0) ? "/" : path.substring(0, lastSlashIndex + 1); } else { String TARGET = argument.startsWith("/") ? argument : (path.endsWith("/") ? path + argument : path + "/" + argument); if (!TARGET.endsWith("/")) { TARGET += "/"; } if (paths.containsKey(TARGET)) { path = TARGET; } else if (TARGET.startsWith("/mnt/")) { try { String REALPWD = "file:///" + TARGET.substring(5); if (!REALPWD.endsWith("/")) REALPWD += "/"; FileConnection fc = (FileConnection) Connector.open(REALPWD, Connector.READ); if (fc.exists() && fc.isDirectory()) { path = TARGET; } else { echoCommand("cd: " + basename(TARGET) + ": not " + (!fc.exists() ? "found" : "a directory")); return 127; } fc.close(); } catch (IOException e) { echoCommand("cd: " + basename(TARGET) + ": " + getCatch(e)); return 1; } } else { echoCommand("cd: " + basename(TARGET) + ": not accessible"); return 127; } } }
        ...
        else if (mainCommand.equals("ls")) { Vector BUFFER = new Vector(); if (path.equals("/mnt/")) { try { Enumeration ROOTS = FileSystemRegistry.listRoots(); while (ROOTS.hasMoreElements()) { String ROOT = (String) ROOTS.nextElement(); if (!BUFFER.contains(ROOT)) { BUFFER.addElement(ROOT); } } } catch (Exception e) { } } else if (path.startsWith("/mnt/")) { try { String REALPWD = "file:///" + path.substring(5); if (!REALPWD.endsWith("/")) { REALPWD += "/"; } FileConnection CONN = (FileConnection) Connector.open(REALPWD, Connector.READ); Enumeration CONTENT = CONN.list(); while (CONTENT.hasMoreElements()) { String ITEM = (String) CONTENT.nextElement(); BUFFER.addElement(ITEM); } CONN.close(); } catch (Exception e) { } } else if (path.equals("/home/") && argument.indexOf("-v") != -1) { try { String[] FILES = RecordStore.listRecordStores(); if (FILES != null) { for (int i = 0; i < FILES.length; i++) { String NAME = FILES[i]; if ((argument.indexOf("-a") != -1 || !NAME.startsWith(".")) && !BUFFER.contains(NAME)) { BUFFER.addElement(NAME); } } } } catch (RecordStoreException e) { } } else if (path.equals("/home/")) { new Explorer(); return 0; } String[] FILES = (String[]) paths.get(path); if (FILES != null) { for (int i = 0; i < FILES.length; i++) { String file = FILES[i].trim(); if (file == null || file.equals("..") || file.equals("/")) { continue; } if (!BUFFER.contains(file) && !BUFFER.contains(file + "/")) { BUFFER.addElement(file); } } } if (!BUFFER.isEmpty()) { StringBuffer FORMATTED = new StringBuffer(); for (int i = 0; i < BUFFER.size(); i++) { String ITEM = (String) BUFFER.elementAt(i); if (!ITEM.equals("/")) { FORMATTED.append(ITEM).append(path.equals("/home/") ? "\n" : "\t"); } } echoCommand(FORMATTED.toString().trim()); } }
        // |
        // Device Files
    ...
    private String substValues(String expr, Hashtable vars, Hashtable program, boolean root) throws RuntimeException {
        ...
        while (true) {
            ...
            String value = call(full, vars, program, false);
    ...
    // | (Building)
    private Hashtable build(String source) { Hashtable program = new Hashtable(); source = removeComments(source).trim(); if (source.equals("")) { return null; } Hashtable functions = new Hashtable(); program.put("functions", functions); while (source.startsWith("#include")) { int endl = source.indexOf("\n"); if (endl == -1) { echoCommand("build: invalid include syntax"); return null; } String line = source.substring(0, endl).trim(); source = source.substring(endl).trim(); if (line.startsWith("#include \"") && line.endsWith("\"")) { String file = extractBetween(line, '"', '"'); Hashtable imported = build(getcontent(file)); if (imported == null) { echoCommand("build: failed to include: " + file); return null; } Hashtable importedFunctions = (Hashtable) imported.get("functions"); for (Enumeration e = importedFunctions.keys(); e.hasMoreElements();) { String k = (String) e.nextElement(); if (!functions.containsKey(k)) functions.put(k, importedFunctions.get(k)); } } else { echoCommand("build: invalid include format: " + line); return null; } } while (true) { int start = findFunctionStart(source); if (start == -1) { break; } int p1 = source.indexOf("(", start), p2 = source.indexOf(")", p1), b1 = source.indexOf("{", p2); String type = source.substring(start, start + source.substring(start).indexOf(" ")).trim(), name = source.substring(start + type.length(), p1).trim(), params = extractBetween(source.substring(p1, p2 + 1), '(', ')'), block = getBlock(source.substring(b1)); if (block == null) { echoCommand("build: incomplete function '" + name + "'"); return null; } source = source.substring(b1 + block.length()).trim(); Hashtable fn = new Hashtable(); fn.put("type", type); Vector reads = new Vector(); if (!params.equals("")) { String[] paramList = split(params, ','); for (int i = 0; i < paramList.length; i++) { String param = paramList[i].trim(); String[] parts = split(param, ' '); if (parts.length != 2) { echoCommand("build: invalid parameter: " + param); return null; } Hashtable arg = new Hashtable(); arg.put("type", parts[0]); arg.put("name", parts[1]); reads.addElement(arg); } } if (!reads.isEmpty()) fn.put("read", reads); block = block.substring(1, block.length() - 1).trim(); fn.put("variables", new Hashtable()); fn.put("source", parseBlock(block, fn)); if (name.equals("main")) { program.put("main", fn); } else { functions.put(name, fn); } } return program; }
    private Vector parseBlock(String block, Hashtable context) { Vector source = new Vector(); String[] lines = splitBlock(block, ';'); for (int i = 0; i < lines.length; i++) { String line = lines[i].trim(); Hashtable cmd = new Hashtable(); if (line.equals("") || line.equals("' '")) { } else if (line.startsWith("return")) { cmd.put("type", "return"); cmd.put("value", line.substring(7).trim()); } else if (line.startsWith("if")) { String type = line.startsWith("if") ? "if" : "else"; int lineIndexInBlock = block.indexOf(line); if (lineIndexInBlock == -1) { echoCommand("build: unable to find '" + line + "' in block"); return null; } int braceIndex = -1; for (int j = lineIndexInBlock; j < block.length(); j++) { if (block.charAt(j) == '{') { braceIndex = j; break; } } if (braceIndex == -1) { echoCommand("build: missing '{' after '" + type + "'"); return null; } String remaining = block.substring(braceIndex), subblock = getBlock(remaining); if (subblock == null) { echoCommand("build: missing block for '" + type + "'"); return null; } cmd.put("type", type); cmd.put("expr", extractParens(line, 0)); int elseIndex = block.indexOf("else", braceIndex + subblock.length()); if (elseIndex != -1) { int elseBrace = block.indexOf("{", elseIndex); if (elseBrace != -1) { String elseSub = getBlock(block.substring(elseBrace)); if (elseSub != null) { cmd.put("else", parseBlock(elseSub.substring(1, elseSub.length() - 1).trim(), context)); } } } cmd.put("source", parseBlock(subblock.substring(1, subblock.length() - 1).trim(), context)); } else if (line.startsWith("while")) { int lineIndexInBlock = block.indexOf(line); if (lineIndexInBlock == -1) { echoCommand("build: unable to find '" + line + "' in block"); return null; } int braceIndex = -1; for (int j = lineIndexInBlock; j < block.length(); j++) { if (block.charAt(j) == '{') { braceIndex = j; break; } } if (braceIndex == -1) { echoCommand("build: missing '{' after 'while'"); return null; } String remaining = block.substring(braceIndex), subblock = getBlock(remaining); if (subblock == null) { echoCommand("build: missing block for 'while'"); return null; } cmd.put("type", "while"); cmd.put("expr", extractParens(line, 0)); cmd.put("source", parseBlock(subblock.substring(1, subblock.length() - 1).trim(), context)); } else if (line.startsWith("else")) { echoCommand("build: invalid token 'else'"); return null; } else if (line.equals("break") || line.equals("continue")) { cmd.put("type", line); } else if (isIsolatedFunctionCall(line)) { String name = line.substring(0, line.indexOf('(')).trim(), arg = extractBetween(line, '(', ')'); cmd.put("type", "call"); cmd.put("function", name); cmd.put("args", arg); } else if (startsWithAny(line, new String[]{"int", "char"})) { String varType = line.startsWith("char ") ? "char" : "int", decls = line.substring(varType.length()).trim(); String[] vars = split(decls, ','); for (int j = 0; j < vars.length; j++) { String part = vars[j].trim(), varName, varValue; int eq = part.indexOf('='); if (eq != -1) { varName = part.substring(0, eq).trim(); varValue = part.substring(eq + 1).trim(); } else { varName = part; varValue = varType.equals("char") ? "' '" : "0"; } if (varName.equals("")) { echoCommand("build: invalid variable declaration: '" + part + "'"); return null; } cmd = new Hashtable(); cmd.put("type", "assign"); cmd.put("name", varName); cmd.put("instance", varType); cmd.put("value", varValue); source.addElement(cmd); } continue; } else if (line.indexOf('=') != -1) { String[] parts = split(line, '='); if (parts.length == 2) { String varName = parts[0].trim(), value = parts[1].trim(); if (varName.indexOf(' ') != -1) { echoCommand("build: invalid type '" + getCommand(varName) + "'"); return null; } cmd.put("type", "assign"); cmd.put("name", varName); cmd.put("value", value); } else { echoCommand("build: invalid value for '" + parts[0].trim() + "'"); return null; } } else { echoCommand("build: invalid syntax: '" + line + "'"); return null; } source.addElement(cmd); } return source; }
    ...
    private int importScript(String script, boolean root) {
        ...
        if (PKG.containsKey("api.version")) {
            String version = env("$VERSION"), apiVersion = (String) PKG.get("api.version");
