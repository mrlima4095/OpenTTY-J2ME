...
public class OpenTTY extends MIDlet implements CommandListener {
    ...
    private void processCommand(String command, boolean ignore) {
        ...
        else if (mainCommand.equals("get")) { if (argument.equals("")) { } else { nanoContent = getcontent(argument); } }
...
public class FTPServer implements Runnable {
    ...
    public void run() {
        try {
            ...
            echoCommand("[+] FTP Server listening at port " + port);

            while (true) {
                SocketConnection clientSocket = (SocketConnection) serverSocket.acceptAndOpen();
                echoCommand("[+] Client connected: " + clientSocket.getAddress());
                new Thread(new FTPClientHandler(clientSocket)).start();
            }
        } catch (IOException e) {
            echoCommand("[-] Server error: " + e.getMessage());
        }
    }

    // Classe interna para tratar cada cliente separadamente
    private class FTPClientHandler implements Runnable {
        ...
        // Para modo ativo
        private String activeDataHost = null;
        private int activeDataPort = -1;

        FTPClientHandler(SocketConnection clientSocket) {
        ...
        public void run() {
            try {
                        ...
                        switch (command) {
                            ...
                            case "CWD":
                                if (changeDirectory(argument)) {
                                    send("250 Directory changed to " + currentPath);
                                } else {
                                    send("550 Access denied or directory not found");
                                }
                                break;
                            ...
                            case "DELE":
                                if (deleteFile(argument)) {
                                    send("250 File deleted");
                                } else {
                                    send("550 File not found or permission denied");
                                }
                                break;
                            ...
                            case "PORT":
                                if (enterActiveMode(argument)) {
                                    send("200 PORT command successful");
                                } else {
                                    send("500 Syntax error in parameters or arguments");
                                }
                                break;
                            ...
                            case "RETR":
                                byte[] fileData = readFile(argument);
                                if (fileData == null) {
                            ...
                            case "STOR":
                                ...
                                if (openDataConnection()) {
                                    ...
                                    if (writeFile(argument, received)) {
                                        closeDataConnection();
                                        send("226 Upload complete");
                                    } else {
                                        closeDataConnection();
                                        send("550 Failed to save file");
                                    }
                                } else {
                                    send("425 Can't open data connection");
                                }
                ...
                echoCommand("[-] Client error: " + e.getMessage());
            } finally {
                closeClient();
            }
        }
        ...
        private boolean changeDirectory(String dir) {
            ...
            if (dir.equals("..")) {
                ...
                // Por simplicidade, vamos só concatenar
                if (!dir.endsWith("/")) dir += "/";
                currentPath += dir;
                return true;
            }
        ...
        private boolean deleteFile(String filename) {
            // Implemente exclusão real, aqui só retorna true para exemplo
            return true;
        }

        private boolean makeDirectory(String dirname) {
            // Implemente criação real, aqui só retorna true para exemplo
            return true;
        }

        private boolean removeDirectory(String dirname) {
            // Implemente remoção real, aqui só retorna true para exemplo
            return true;
        }
        ...
        private void enterPassiveMode() throws IOException {
            ...
            // Porta dinâmica entre 1025 e 65535 (exemplo fixo 2121 para simplicidade)
            int pasvPort = 2121;  // Você pode randomizar ou usar uma faixa configurada
            dataServerSocket = (ServerSocketConnection) Connector.open("socket://:" + pasvPort);
            ...
            // Para modo passivo: abrirá conexão com cliente via dataServerSocket.acceptAndOpen()
            activeDataHost = null; // limpa modo ativo
            activeDataPort = -1;
        }

        private boolean enterActiveMode(String argument) {
            try {
                // Exemplo argumento: "192,168,1,100,7,138"
        ...
        private boolean openDataConnection() {
            try {
                if (activeDataHost != null && activeDataPort != -1) {
                    // Modo ativo: conecta ao cliente na porta especificada
                    dataConnection = (SocketConnection) Connector.open("socket://" + activeDataHost + ":" + activeDataPort);
                } else if (dataServerSocket != null) {
                    // Modo passivo: aguarda conexão do cliente
        ...
        private void closeDataConnection() {
            try {
                ...
                if (dataServerSocket != null && activeDataHost == null) {
                    // Fecha dataServerSocket apenas no modo passivo
                    dataServerSocket.close();
                    dataServerSocket = null;
                }
        ...
        private void closeClient() {
            try {
                ...
                closeDataConnection();
                echoCommand("[-] Client disconnected");
            } catch (IOException ignored) {}
        }
        ...
        private String getArgument(String line) {
            int space = line.indexOf(' ');
            return (space == -1) ? "" : line.substring(space + 1).trim();
        }

        // Exemplo simples para obter lista de diretório
        private String getDirectoryListing() {
            // Implementar listagem real do diretório atual
            // Aqui, exemplo estático
            return "-rw-r--r-- 1 user group 1024 Jan 01 12:00 file1.txt\r\n"
                 + "drwxr-xr-x 1 user group 0 Jan 01 12:00 dir1\r\n";
        }

        // Exemplo leitura de arquivo - implemente leitura real
        private byte[] readFile(String filename) {
            // Implementar leitura real
            // Aqui só retorna conteúdo fixo para teste
            if ("file1.txt".equals(filename)) {
                return "Conteúdo do arquivo file1.txt\n".getBytes();
            }
            return null;
        }

        // Exemplo gravação de arquivo - implemente gravação real
        private boolean writeFile(String filename, byte[] data) {
            // Implementar gravação real
            // Aqui apenas aceita sem fazer nada
            return true;
        }

        // Ler todos os dados enviados pelo cliente no modo STOR
        private byte[] readData() throws IOException {
            ...
            while ((len = dataIs.read(buffer)) != -1) {
                ...
                // Normalmente cliente fecha a conexão após envio, então -1 será retornado
            }
    ...
    private void echoCommand(String msg) {
        // Implemente seu log, console ou debug
