...
public class OpenTTY extends MIDlet implements CommandListener {
    ...
    private int processCommand(String command, boolean ignore, boolean root) { 
        ...
        // Directories Manager
            ...
            if (PWD.equals("/mnt/")) { try { for (Enumeration ROOTS = FileSystemRegistry.listRoots(); ROOTS.hasMoreElements();) { String ROOT = (String) ROOTS.nextElement(); if (!BUFFER.contains(ROOT)) { BUFFER.addElement(ROOT); } } } catch (Exception e) { } } 
            else if (PWD.startsWith("/mnt/")) { try { String REALPWD = "file:///" + PWD.substring(5); if (!REALPWD.endsWith("/")) { REALPWD += "/"; } FileConnection CONN = (FileConnection) Connector.open(REALPWD, Connector.READ); for (Enumeration CONTENT = CONN.list(); CONTENT.hasMoreElements();) { String ITEM = (String) CONTENT.nextElement(); BUFFER.addElement(ITEM); } CONN.close(); } catch (Exception e) { } } 
            else if (PWD.equals("/home/") && argument.indexOf("-v") != -1) { try { String[] FILES = RecordStore.listRecordStores(); if (FILES != null) { for (int i = 0; i < FILES.length; i++) { String NAME = FILES[i]; if ((argument.indexOf("-a") != -1 || !NAME.startsWith(".")) && !BUFFER.contains(NAME)) { BUFFER.addElement(NAME); } } } } catch (RecordStoreException e) { } } 
            else if (PWD.equals("/home/")) { new Explorer(); return 0; } 

            String[] FILES = (String[]) paths.get(PWD); if (FILES != null) { for (int i = 0; i < FILES.length; i++) { String file = FILES[i].trim(); if (file == null || file.equals("..") || file.equals("/")) { continue; } if (!BUFFER.contains(file) && !BUFFER.contains(file + "/")) { BUFFER.addElement(file); } } } 
            if (BUFFER.isEmpty()) { }
            else {
            ...
            if (argument.equals("")) { } 
                ...
                if (NODES.containsKey(argument)) { 
                    try { 
                        ...
                        else if (argument.equals("socket")) { ((SocketConnection) Connector.open(env("socket://" + (attributes.containsKey("REPO") && !(env("$REPO").equals("")) ? "$REPO" : "1.1.1.1:53")))).close(); } 
                ...
                else if (argument.equals("*")) { for (Enumeration KEYS = NODES.keys(); KEYS.hasMoreElements();) { STATUS = processCommand("chmod " + (String) KEYS.nextElement(), false); } }
