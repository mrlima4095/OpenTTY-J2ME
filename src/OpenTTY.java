...
public class OpenTTY extends MIDlet implements CommandListener {
    ...
    private String exprCommand(String expr) { char[] t = expr.toCharArray(); double[] v = new double[32]; char[] o = new char[32]; int vt = -1, ot = -1, i = 0, l = t.length; while (i < l) { char c = t[i]; if (c == ' ') { i++; continue; } if (c >= '0' && c <= '9') { double n = 0; while (i < l && t[i] >= '0' && t[i] <= '9') { n = n * 10 + (t[i++] - '0'); } if (i < l && t[i] == '.') { i++; double f = 0, d = 10; while (i < l && t[i] >= '0' && t[i] <= '9') { f += (t[i++] - '0') / d; d *= 10; } n += f; } v[++vt] = n; } else if (c == '(') { o[++ot] = c; i++; } else if (c == ')') { while (ot >= 0 && o[ot] != '(') { double b = v[vt--], a = v[vt--]; char op = o[ot--]; v[++vt] = applyOpSimple(op, a, b); } ot--; i++; } else if (c == '+' || c == '-' || c == '*' || c == '/' || c == '%') { while (ot >= 0 && prec(o[ot]) >= prec(c)) { double b = v[vt--], a = v[vt--]; char op = o[ot--]; v[++vt] = applyOpSimple(op, a, b); } o[++ot] = c; i++; } else { return "expr: invalid char '" + c + "'"; } } while (ot >= 0) { double b = v[vt--], a = v[vt--]; char op = o[ot--]; v[++vt] = applyOpSimple(op, a, b); } double r = v[vt]; return ((int) r == r) ? String.valueOf((int) r) : String.valueOf(r); } private int prec(char op) { if (op == '+' || op == '-') return 1; if (op == '*' || op == '/' || op == '%') return 2; return 0; } private double applyOpSimple(char op, double a, double b) { if (op == '+') return a + b; if (op == '-') return a - b; if (op == '*') return a * b; if (op == '/') return b == 0 ? 0 : a / b; if (op == '%') return b == 0 ? 0 : a % b; return 0; }
    ...
    private int C2ME(String code, boolean root) {
        Hashtable program = build(code.startsWith("int main()") ? code : getcontent(code));
    ...
    private String call(String code, Hashtable vars, Hashtable program, boolean root) throws RuntimeException {
        ...
        if (fname.equals("printf")) {
            if (argList.length < 1 || argList.length > 2) { throw new RuntimeException("function '" + fname + "' expects 1 argument(s), got " + argList.length); }
            else {
    ...
    private String format(String expr) throws RuntimeException {
        ...
        if (expr.startsWith("\"") && expr.endsWith("\"") || expr.startsWith("'") && expr.endsWith("'")) { return env(expr.substring(1, expr.length() - 1)); }
