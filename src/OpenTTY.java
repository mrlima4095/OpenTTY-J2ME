...
public class OpenTTY extends MIDlet implements CommandListener {
    ...
    private int xserver(String command, boolean root) {
        ...
        else if (mainCommand.equals("init")) { form.setTitle(env("OpenTTY $VERSION")); form.append(stdout); form.append(stdin); form.addCommand(EXECUTE); xserver("cmd", true); form.setCommandListener(this); }
        else if (mainCommand.equals("stop")) { form.setTitle(null); form.setTicker(null); form.deleteAll(); xserver("cmd hide", root); form.removeCommand(EXECUTE); }
