...
public class OpenTTY extends MIDlet implements CommandListener {
    ...
    public void startApp() {
        if (!trace.containsKey("sh")) {
            attributes.put("PATCH", "UI Update"); attributes.put("VERSION", getAppProperty("MIDlet-Version")); attributes.put("RELEASE", "beta"); attributes.put("XVERSION", "0.6");
    ...
    //public class MyTaskManager implements CommandListener { private List tasks = new List(form.getTitle(), List.IMPLICIT); private TextBox taskname = new TextBox("Create Task", "", 256, TextField.ANY); private Alert clearConfirm = new Alert(form.getTitle(), "Are you sure?\n\nYou'll delete all of your tasks", null, AlertType.WARNING); private Command backCommand = new Command("Back", Command.BACK, 1), saveCommand = new Command("Save", Command.OK, 1), newCommand = new Command("New Task", Command.SCREEN, 2), readCommand = new Command("View more", Command.SCREEN, 3), toggleCommand = new Command("Toggle Status", Command.SCREEN, 4), clearCommand = new Command("Delete all", Command.SCREEN, 5), deleteCommand = new Command("Delete", Command.SCREEN, 6); private String file = ".tasks"; public MyTaskManager() { tasks.addCommand(backCommand); tasks.addCommand(newCommand); tasks.addCommand(toggleCommand); tasks.addCommand(readCommand); tasks.addCommand(clearCommand); tasks.addCommand(deleteCommand); taskname.addCommand(saveCommand); clearConfirm.addCommand(new Command("Yes", Command.OK, 1)); clearConfirm.addCommand(new Command("No", Command.BACK, 2)); tasks.setCommandListener(this); taskname.setCommandListener(this); clearConfirm.setCommandListener(this); readTasks(); display.setCurrent(tasks); } public void commandAction(Command c, Displayable d) { if (display.getCurrent() == tasks) { if (c == backCommand) { processCommand("xterm"); } else if (c == newCommand) { display.setCurrent(taskname); } else if (c == toggleCommand) { toggleTask(tasks.getString(tasks.getSelectedIndex())); } else if (c == readCommand) { viewmore(tasks.getString(tasks.getSelectedIndex())); } else if (c == clearCommand) { display.setCurrent(clearConfirm); } else if (c == deleteCommand) { deleteTask(tasks.getString(tasks.getSelectedIndex())); } } else if (display.getCurrent() == taskname) { if (c == saveCommand) { if (taskname.getString().equals("")) { display.setCurrent(tasks); } else { writeTasks(); readTasks(); taskname.setString(""); display.setCurrent(tasks); } } } else if (display.getCurrent() == clearConfirm) { if (c.getLabel() == "Yes") { writeRMS(file, ""); readTasks(); display.setCurrent(tasks); } else if (c.getLabel() == "No") { display.setCurrent(tasks); } } } private void writeTasks() { writeRMS(file, loadRMS(file, 1) + "\n" + taskname.getString()); } private void readTasks() { tasks.deleteAll(); String[] tasklist = split(loadRMS(file, 1), '\n'); for (int i = 0; i < tasklist.length; i++) { if (!tasklist[i].trim().equals("")) { tasks.append(tasklist[i].trim(), null); } } } private void deleteTask(String task) { String[] tasklist = split(loadRMS(file, 1), '\n'); StringBuffer newlist = new StringBuffer(); for (int i = 0; i < tasklist.length; i++) { if (task.equals(tasklist[i])) { } else { newlist.append(tasklist[i] + "\n"); } } writeRMS(file, newlist.toString()); readTasks(); } private void toggleTask(String task) { String[] tasklist = split(loadRMS(file, 1), '\n'); StringBuffer newlist = new StringBuffer(); for (int i = 0; i < tasklist.length; i++) { if (tasklist[i].equals(task)) { if (task.startsWith("[COMPLETE] ")) { newlist.append(task.substring("[COMPLETE] ".length()).trim() + "\n"); } else { newlist.append("[COMPLETE] " + task + "\n"); } } else { newlist.append(tasklist[i] + "\n"); } } writeRMS(file, newlist.toString()); readTasks(); } private void viewmore(String task) { String status = "Needs action"; if (task.startsWith("[COMPLETE] ")) { status = "Finished"; } warnCommand(form.getTitle(), env("Task Name: " + replace(task, "[COMPLETE] ", "") + "\nStatus: " + status)); } } 
    public class MyTaskManager implements CommandListener { 
        private List tasks = new List(form.getTitle(), List.IMPLICIT); private TextBox taskname = new TextBox("Create Task", "", 256, TextField.ANY); private Alert clearConfirm = new Alert(form.getTitle(), "Are you sure?\n\nYou'll delete all of your tasks", null, AlertType.WARNING); private Command backCommand = new Command("Back", Command.BACK, 1), saveCommand = new Command("Save", Command.OK, 1), newCommand = new Command("New Task", Command.SCREEN, 2), readCommand = new Command("View more", Command.SCREEN, 3), toggleCommand = new Command("Toggle Status", Command.SCREEN, 4), clearCommand = new Command("Delete all", Command.SCREEN, 5), deleteCommand = new Command("Delete", Command.SCREEN, 6); private String file = ".tasks"; 
        public MyTaskManager() { tasks.addCommand(backCommand); tasks.addCommand(newCommand); tasks.addCommand(toggleCommand); tasks.addCommand(readCommand); tasks.addCommand(clearCommand); tasks.addCommand(deleteCommand); taskname.addCommand(saveCommand); clearConfirm.addCommand(new Command("Yes", Command.OK, 1)); clearConfirm.addCommand(new Command("No", Command.BACK, 2)); tasks.setCommandListener(this); taskname.setCommandListener(this); clearConfirm.setCommandListener(this); readTasks(); display.setCurrent(tasks); } 
        public void commandAction(Command c, Displayable d) { if (display.getCurrent() == tasks) { if (c == backCommand) { processCommand("xterm"); } else if (c == newCommand) { display.setCurrent(taskname); } else if (c == toggleCommand) { toggleTask(tasks.getString(tasks.getSelectedIndex())); } else if (c == readCommand) { viewmore(tasks.getString(tasks.getSelectedIndex())); } else if (c == clearCommand) { display.setCurrent(clearConfirm); } else if (c == deleteCommand) { deleteTask(tasks.getString(tasks.getSelectedIndex())); } } else if (display.getCurrent() == taskname) { if (c == saveCommand) { if (taskname.getString().equals("")) { display.setCurrent(tasks); } else { writeTasks(); readTasks(); taskname.setString(""); display.setCurrent(tasks); } } } else if (display.getCurrent() == clearConfirm) { if (c.getLabel() == "Yes") { writeRMS(file, ""); readTasks(); display.setCurrent(tasks); } else if (c.getLabel() == "No") { display.setCurrent(tasks); } } } private void writeTasks() { writeRMS(file, loadRMS(file, 1) + "\n" + taskname.getString()); } 
        ...
        private void viewmore(String task) { String status = "Needs action"; if (task.startsWith("[COMPLETE] ")) { status = "Finished"; } else if (task.startsWith("[WORKING] ")) { status = "Working"; } warnCommand(form.getTitle(), env("Task Name: " + replace(task, "[COMPLETE] ", "") + "\nStatus: " + status)); } } 
