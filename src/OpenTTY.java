...
public class OpenTTY extends MIDlet implements CommandListener {
    ...
    private void processCommand(String command, boolean ignore) {
        ...
        // Directories Manager
            ...
            if (argument.equals("")) { echoCommand(readStack() == null || readStack().length() == 0 ? "pushd: missing directory": readStack()); } 
                ...
                else { stack.addElement(path); path = argument; echoCommand(readStack()); }
            }
        }
        else if (mainCommand.equals("popd")) { if (stack.isEmpty()) { echoCommand("popd: stack empty"); } else { path = (String) stack.lastElement(); stack.removeElementAt(stack.size() - 1); echoCommand(path); } }
    ...
    private String readStack() { StringBuffer sb = new StringBuffer(); sb.append(path); for (int i = 0; i < dirStack.size(); i++) { sb.append(" ").append((String) dirStack.elementAt(i)); } return sb.toString(); }
    // |
    // RMS Files
