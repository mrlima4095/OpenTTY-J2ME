...
class Lua {
    ...
    public class LuaFunction {
        ...
        public Object internals(Vector args) throws Exception {
            if (MOD == PRINT || MOD == EXEC || MOD == GC) { if (args.isEmpty()) { } else { return midlet.processCommand(MOD == GC ? "gc" : (MOD == PRINT ? "echo " : "") + toLuaString(args.elementAt(0)), true, root); } }
            ...
            else if (MOD == HASH) { return args.isEmpty() || args.elementAt(0) == null ? new Double(args.elementAt(0).hashCode()) : null; }
            else if (MOD == BYTE) {
                ...
                if (args.isEmpty() || args.elementAt(0) == null) { throw new Exception("select: index expected"); }
                    ...
                    if (idx.equals("#")) { return new Double(args.size() - 1); }
                        ...
                        Hashtable result = new Hashtable();
                        for (int i = 1; i <= end; i++) { result.put(new Double(i), args.elementAt(i)); }
