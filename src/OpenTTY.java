...
class Lua {
    ...
    // |
    public static final int PRINT = 0, EXEC = 1, ERROR = 2, PCALL = 3, GETENV = 4, REQUIRE = 5, CLOCK = 6, EXIT = 7, SETLOC = 8, PAIRS = 9, READ = 10, WRITE = 11, GC = 12, TOSTRING = 13, TONUMBER = 14, RAWLEN = 15, RAWGET = 16;
        ...
        public Object internals(Vector args) throws Exception {
            if (MOD == PRINT || MOD == EXEC || MOD == GC) { if (args.isEmpty()) { } else { return midlet.processCommand(MOD == GC ? "gc" : (MOD == PRINT ? "echo " : "") + toLuaString(args.elementAt(0)), true, root); } }
                ...
                if (args.isEmpty()) { }
                    ...
                    Object obj = args.elementAt(0), ret = MOD == RAWGET ? closureScope.get(obj) : new Integer(obj instanceof Hashtable ? ((Hashtable) obj).size() : obj instanceof String ? ((String) obj).length() : 0);
