...
public class OpenTTY extends MIDlet implements CommandListener {
    ...
    public void commandAction(Command c, Displayable d) {
        if (d == nano) {
            ...
            else { processCommand(c == HELP ? "help" : c == NANO ? "nano" : c == CLEAR ? "clear" : c == HISTORY ? "history" : BACK ? "xterm" : "exit"); }
        }
    }
    private int load(int ITEM) {
        if (ITEM == HISTORY) { preview.deleteAll(); for (int i = 0; i < history.size(); i++) { preview.append((String) history.elementAt(i), null); } } 
            ...
            try { 
                if (path.equals("/mnt/")) { 
                    ...
                    while (roots.hasMoreElements()) { explorer.append((String) roots.nextElement(), DIR); } 
                } 
                ...
                if (files != null) { 
                    for (int i = 0; i < files.length; i++) { 
                        ...
                        if (f != null && !f.equals("..") && !f.equals("/")) { 
                            explorer.append(f, f.endsWith("/") ? DIR : FILE);
                        } 
                    } 
                } 
            } catch (IOException e) { } 
