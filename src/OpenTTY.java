...
public class OpenTTY extends MIDlet implements CommandListener {
    ...
    public class MIDletControl implements ItemCommandListener, CommandListener, Runnable {
        ...
        if (asking_user) { monitor.append(USER = new TextField("Username", "", 256, TextField.ANY)); } if (asking_passwd) { monitor.append(PASSWD = new TextField("Password", "", 256, TextField.ANY | TextField.PASSWORD)); } monitor.addCommand(LOGIN = new Command("Login", Command.OK, 1)); monitor.addCommand(EXIT = new Command("Exit", Command.SCREEN, 2)); } monitor.setCommandListener(this); display.setCurrent(monitor); } }
        ...
        public MIDletControl(String file, int id, Object stdout, Hashtable scope) {
            MOD = NANO; this.filename = file; this.id = id; this.stdout; this.scope = scope;
            
            box = ("Nano - " + (file == null || file.equals("") ? " New Buffer"), file == null || file.equals("") ? "" : getcontent(file), 31522, TextField.ANY);
            box.addCommand(BACK);
    ...
    public int processCommand(String command, boolean enable, int id, String pid, Object stdout, Hashtable scope) {
        ...
        else if (mainCommand.equals("ping")) { if (argument.equals("")) { } else { long START = System.currentTimeMillis(); try { HttpConnection CONN = (HttpConnection) Connector.open(!argument.startsWith("http://") && !argument.startsWith("https://") ? "http://" + argument : argument); CONN.setRequestMethod(HttpConnection.GET); int responseCode = CONN.getResponseCode(); CONN.close(); echoCommand("Ping to " + argument + " successful, time=" + (System.currentTimeMillis() - START) + "ms"); } catch (IOException e) { print("Ping to " + argument + " failed: " + getCatch(e), stdout); return 101; } } }
        // |
        ...
        else if (mainCommand.equals("ttysize")) { print((stdout instanceof StringItem ? stdout.getText().length() : stdout instanceof String ? read(stdout).length() : "-1") + " B", stdout); }
        ...
        else if (mainCommand.startsWith("@")) { print("mod: " + mainComand.substring(1) + ": not found", stdout); }
        // |
