...
public class OpenTTY extends MIDlet implements CommandListener {
    ...
    public void startApp() {
        if (!trace.containsKey("sh")) {
            attributes.put("PATCH", "Runtime Update"); attributes.put("VERSION", getAppProperty("MIDlet-Version")); attributes.put("RELEASE", "stable"); attributes.put("XVERSION", "0.6");
    ...
    private void clone(String node) {
    ...
    public class MyTaskManager implements CommandListener {
        ...
        private void readTasks() {
            tasks.deleteAll();
            String[] tasklist = split(loadRMS(file, 1), '\n');
            for (int i = 0; i < tasklist.length; i++) {
                if (!tasklist[i].trim().equals("")) {
                    tasks.append(tasklist[i].trim(), null);
                }
            }
        }
        ...
        private void deleteTask(String task) {
            ...
            for (int i = 0; i < tasklist.length; i++) {
                if (!task.equals(tasklist[i]) && !task.equals("")) {
                    newlist.append(tasklist[i] + "\n");
                }
            }
            
            writeRMS(file, newlist.toString().trim());
            readTasks();
        }
        ...
        private void clearFinished() {
            ...
            for (int i = 0; i < tasklist.length; i++) {
                String task = tasklist[i].trim();
                if (!task.startsWith("[COMPLETE] ") && !task.equals("")) {
                    newlist.append(task + "\n");
                }
            }
            ...
            for (int i = 0; i < tasklist.length; i++) {
                if (tasklist[i].equals(task)) {
                    if (task.startsWith("[COMPLETE] ")) { newlist.append(task.substring("[COMPLETE] ".length()).trim() + "\n"); } 
                        ...
                        if (task.endsWith("*")) { newlist.append("[COMPLETE] " + task + "\n"); } 
        ...
        private void viewmore(String task) {
            String status = "Needs action";
            
            if (task.startsWith("[COMPLETE] ")) { status = "Finished"; } 
            else if (task.startsWith("[WORKING] ")) { status = "Working"; }
            
            warnCommand(form.getTitle(), env("Task Name: " + replace(replace(task, "[COMPLETE] ", ""), "[WORKING] ", "") + "\nStatus: " + status));
        }
    }
