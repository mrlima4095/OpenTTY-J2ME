...
public class OpenTTY extends MIDlet implements CommandListener {
    ...
    private void processCommand(String command, boolean ignore) {
        ...
        //else if (mainCommand.equals("")) {  }
        //else if (mainCommand.equals("")) {  }
        else if (mainCommand.equals("@flash")) { display.flashBacklight(100); }
    ...
    public class Contacts implements CommandListener {
        private List agenda = new List(form.getTitle(), List.IMPLICIT); 
        private Command backCommand = new Command("Back", Command.BACK, 1), callCommand = new Command("Call", Command.SCREEN, 2), sendCommand = new Command("Send SMS", Command.SCREEN, 3);

        public Contacts() {

            agenda.addCommand(backCommand);
            agenda.addCommand(callCommand);
            agenda.addCommand(sendCommand);
            ...
            try {
                ...
                while (contacts.hasMoreElements()) {
                    ...
                    String name = contact.countValues(Contact.FORMATTED_NAME) > 0
                            ? contact.getString(Contact.FORMATTED_NAME, 0)
                            : "Unknown";
                    String number = contact.countValues(Contact.TEL) > 0
                            ? contact.getString(Contact.TEL, 0)
                            : "No Number";

                    agenda.append(name + " - " + number, null);
                }
                cl.close();
            } catch (Exception e) {
                echoCommand(e.getMessage()); return;
            }
        ...
        public void commandAction(Command c, Displayable d) {
            ...
            } else if (c == callCommand || c == smsCommand) {
                int index = agenda.getSelectedIndex();
                if (index != -1) {
                    String selected = agenda.getString(index);
                    selectedNumber = selected.substring(selected.lastIndexOf(" - ") + 3);

                    if (c == callCommand) {
                        try {
                            platformRequest("tel:" + selectedNumber);
                        } catch (Exception e) {
                            processCommand("builtin warn " + e.getMessage());
                        }
                    } else if (c == smsCommand) {
                        try {
                            platformRequest("sms:" + selectedNumber);
                        } catch (Exception e) {
                            processCommand("builtin warn " + e.getMessage());
                        }
                    }
