...
class Lua {
    ...
    public class LuaFunction implements CommandListener {
        ...
        public Object internals(Vector args) throws Exception {
            ...
            else if (MOD == EXIT) { if (args.isEmpty()) { throw new Error(); } else { status = midlet.getNumber(toLuaString(args.elementAt(0)), 1, false); midlet.trace.remove(PID); } }
            else if (MOD == READ) {
        ...
        private String getvalue(Hashtable table, String key, String fallback) { return table.containsKey(key) ? (String) table.get(key) : fallback; } 
                        ...
                        for (Enumeration keys = fields.keys(); keys.hasMoreElements();) {
                            ...
                            if (fieldObj instanceof Hashtable) {
                                ...
                                if (type.equals("image")) {
                                    ...
                                    int w = field.containsKey("width") ? field.get("width") instanceof Double ? ((Double) field.get("width")).intValue() : (int) gotbad(1, "width", "number expected, got " + type(field.get("width"))) : 1;
                                    int h = field.containsKey("heigth") ? field.get("heigth") instanceof Double ? ((Double) field.get("heigth")).intValue() : (int) gotbad(1, "heigth", "number expected, got " + type(field.get("heigth"))) : 10;
