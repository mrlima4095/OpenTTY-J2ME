...
public class OpenTTY extends MIDlet implements CommandListener {
    ...
    private int query(String command, int id) { 
        ...
        if (mainCommand.equals("")) { echoCommand("query: missing [address]"); return 2; } 
            ...
            try { 
                ...
                int LENGTH = IN.read(BUFFER); 
                String DATA = LENGTH > 0 ? new String(BUFFER, 0, LENGTH, "UTF-8") : "", FILE = env("$QUERY"); 
    ...
    public int writeRMS(String filename, byte[] data, int id) { 
        if (filename == null || filename.length() == 0) { return 2; } 
            ...
            else { cache.put("/" + base + "/" + filename, new String(data)); return addFile(filename, new String(data), loadRMS("OpenRMS", base.equals("bin") ? 3 : 4), base, id); }
        }
    ...
    public int addFile(String filename, String content, String archive, String base, int id) { 
        return writeRMS(
            "OpenRMS", 
            (delFile(filename, archive) + "[\0BEGIN:" + filename + "\0]\n" + content + "\n[\0END\0]\n").getBytes(), 
            base.equals("bin") ? 3 : 4, 
            id
        ); 
    }
