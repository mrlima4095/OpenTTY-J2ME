...
public class OpenTTY extends MIDlet implements CommandListener {
    ...
    public void startApp() {
        if (!app == true) {
            ...
            attributes.put("PATCH", "API Update"); attributes.put("VERSION", getAppProperty("MIDlet-Version")); attributes.put("RELEASE", "stable"); attributes.put("XVERSION", "0.5");
    ...
    private void processCommand(String command, boolean ignore) {
        ...
        else if (mainCommand.equals("dd")) { if (argument == null || argument.length() == 0) return; String[] args = split(argument, ' '); try (FileConnection fileConn = (FileConnection) Connector.open("file:///" + args[0], Connector.READ_WRITE); OutputStream os = fileConn.exists() ? fileConn.openOutputStream() : (fileConn.create() ? fileConn.openOutputStream() : null)) { String content = args[1]; os.write(content.startsWith("/") ? read(content).getBytes() : (content.equals("nano") ? nanoContent.getBytes() : loadRMS(content, 1).getBytes())); os.flush(); echoCommand("operation finish"); } catch (IOException e) { echoCommand(e.getMessage()); } }
