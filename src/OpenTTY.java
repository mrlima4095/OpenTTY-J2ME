...
public class OpenTTY extends MIDlet implements CommandListener {
    ...
    private int processCommand(String command, boolean ignore, boolean root) { 
        ...
        else if (mainCommand.equals("sh") || mainCommand.equals("login")) { return processCommand(argument.equals("") ? "import /java/bin/sh" : ". " + argument, false); }
        ...
        else if (equals(mainCommand, new String[]{ "run", "." })) { return runScript(argument.equals("") ? nanoContent : getcontent(argument), root); }
    ...
    private boolean equals(String command, String[] patterns) { for (int i = 0; i < patterns.length; i++) { if (command.equals(patterns[i])) { return true; } } return false; }
        ...
        public void commandAction(Command c, Displayable d) { 
            if (c == LOGIN) { 
                ...
                if (TYPE == SIGNUP) { 
                    ...
                    loadScreen(form); runScript(loadRMS("initd")); 
                } 
            } 
            else if (TYPE == REQUEST) { 
                if (password.equals("")) { warnCommand(form.getTitle(), "Missing credentials!"); } 
                else { if (String.valueOf(password.hashCode()).equals(passwd(false, null))) { processCommand("xterm"); processCommand(command, true, true); } else { PASSWD.setString(""); warnCommand(form.getTitle(), "Wrong password!"); } } } stdin.setLabel(username + " " + path + " " + (username.equals("root") ? "#" : "$")); } else if (c == EXIT) { processCommand(TYPE == REQUEST ? "xterm" : "exit", false); } } }
    ...
    public class Screen implements CommandListener { 
        ...
        public Screen(String type, String code) { 
            ...
            if (type.equals("make")) { 
                ...
                list.setCommandListener(this); loadScreen(list); 
            } 
                ...
                edit.setCommandListener(this); loadScreen(edit);
            ...
            if (PKG.containsKey("canvas.fields")) { 
                for (int i = 0; i < fields.size(); i++) { 
                    ...
                    if (type.equals("text")) { 
                        ...
                        catch (IOException e) { MIDletLogs("add error Malformed Image, " + getCatch(e)); } 
                    } 
    ...
    private int stop(String app, boolean root) {
        ...
        for (Enumeration keys = trace.keys(); keys.hasMoreElements();) {
            ...
            if (app.equals(NAME)) {
                if ((STATUS = kill(PID, false, root)) != 0) { break; }
            }
        }
    ...
    public class Server implements Runnable {
        private int TYPE = 0, SERVER = 1, BIND = 2, COUNT = 1;

        private boolean root;
        private String port, mod, service;

        public Server(String mode, String args, boolean root) {
            if (args == null || args.length() == 0 || args.equals("$PORT")) { processCommand("set PORT=31522", false); new Server(mode, "31522", root); return; }
    ...
    public class RemoteConnection implements CommandListener, Runnable {
        private int TYPE, NC = 1, PRSCAN = 2, GOBUSTER = 3;
