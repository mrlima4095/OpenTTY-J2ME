...
public class OpenTTY extends MIDlet implements CommandListener {
    ...
    private void processCommand(String command, boolean ignore) {
        ...
        else if (mainCommand.equals("pprt")) { try { SocketConnection socketConnection = (SocketConnection) Connector.open("socket://1.1.1.1:53"); echoCommand(socketConnection.getLocalPort().toString()); socketConnection.close(); } catch (IOException e) { echoCommand("null"); } }
        else if (mainCommand.equals("pprt2")) { try { SocketConnection socketConnection = (SocketConnection) Connector.open("socket://1.1.1.1:53"); echoCommand(socketConnection.getRemotePort().toString()); socketConnection.close(); } catch (IOException e) { echoCommand("null"); } }
