...
public class OpenTTY extends MIDlet implements CommandListener {
    ...
    // Network API Service
    private void netstat() { try { HttpConnection conn = (HttpConnection) Connector.open("http://ipinfo.io/ip"); conn.setRequestMethod(HttpConnection.GET); InputStream is = conn.openInputStream(); ByteArrayOutputStream baos = new ByteArrayOutputStream(); int ch; while ((ch = is.read()) != -1) { baos.write(ch); } echoCommand("true"); conn.close(); } catch (Exception e) { echoCommand("false"); } }
    private void fwCommand(String ip) { final String url = "http://ipinfo.io/" + (ip == null || ip.length() == 0 ? "json" : ip); try { HttpConnection conn = (HttpConnection) Connector.open(url); conn.setRequestMethod(HttpConnection.GET); InputStream is = conn.openInputStream(); ByteArrayOutputStream baos = new ByteArrayOutputStream(); int ch; while ((ch = is.read()) != -1) { baos.write(ch); } String response = new String(baos.toByteArray()); echoCommand(response); is.close(); conn.close(); } catch (Exception e) { echoCommand(e.getMessage()); } }
    private void curlCommand(String url) { if (url == null || url.length() == 0) { return; } if (!url.startsWith("http://") && !url.startsWith("https://")) { url = "http://" + url; } try { HttpConnection conn = (HttpConnection) Connector.open(url); conn.setRequestMethod(HttpConnection.GET); InputStream is = conn.openInputStream(); ByteArrayOutputStream baos = new ByteArrayOutputStream(); int ch; while ((ch = is.read()) != -1) { baos.write(ch); } echoCommand(new String(baos.toByteArray())); is.close(); conn.close(); } catch (Exception e) { echoCommand(e.getMessage()); } }
    private void wgetCommand(String url) { if (url == null || url.length() == 0) { return; } if (!url.startsWith("http://") && !url.startsWith("https://")) { url = "http://" + url; } try { HttpConnection conn = (HttpConnection) Connector.open(url); conn.setRequestMethod(HttpConnection.GET); InputStream is = conn.openInputStream(); ByteArrayOutputStream baos = new ByteArrayOutputStream(); int ch; while ((ch = is.read()) != -1) { baos.write(ch); } nanoContent = new String(baos.toByteArray()); echoCommand("wget: html saved to nano"); is.close(); conn.close(); } catch (Exception e) { echoCommand(e.getMessage()); } }
    ...
    private void portScanner(final String host) { 
        ...
        final List ports = new List(host + " Ports", List.IMPLICIT); 
        final Command connectCommand = new Command("Connect", Command.OK, 1); 
        final Command backCommand = new Command("Back", Command.BACK, 2); 

        ports.addCommand(connectCommand); ports.addCommand(backCommand); 
        ports.setCommandListener(new CommandListener() { public void commandAction(Command c, Displayable d) { 
                if (c == connectCommand) { 
                    connect(host + ":" + ports.getString(ports.getSelectedIndex())); 
