...
public class OpenTTY extends MIDlet implements CommandListener {
    ...
    private int java(String command) {
        ...
        if (mainCommand.equals("")) { viewer("Java ME", env("Java 1.2 (OpenTTY Edition)\n\nMicroEdition-Config: $CONFIG\nMicroEdition-Profile: $PROFILE")); }
            ...
            try {
                byte[] classBytes = getcontent(className).getBytes("ISO-8859-1");
                ByteArrayInputStream bais = new ByteArrayInputStream(classBytes);
                ClassFile classFile = new ClassFile(bais); // seu parser de .class
                classFile.invokeMain(); 
            } catch (Exception e) {
                echoCommand("java: " + className + ": " + e.getMessage());
                return 1;
            }
        } 
    ...
    private byte[] generateClassBytes(String className, String code) {
        ...
        try {
            ...
            out.write(new byte[] { 0x01, 0x00, 0x06, 'u','n','u','s','e','d' });
