...
public class OpenTTY extends MIDlet implements CommandListener {
    ...
    /* public class Bluetooh  {
        ...
        public Bluetooh(String args) {
            command = env(args.trim());
            String mainCommand = getCommand(command).toLowerCase();
            String argument = getArgument(command);
            try {
                this.localDevice = LocalDevice.getLocalDevice();  // Obtém o dispositivo Bluetooth local
                this.discoveryAgent = localDevice.getDiscoveryAgent();  // Obtém o agente de descoberta
                ...
                if (mainCommand.equals("conn")) {
                    connectToDevice(argument);
                ...
                System.out.println("Erro ao inicializar o dispositivo Bluetooth: " + e.getMessage());
            }
        }

        
        // Comando para se conectar a um dispositivo Bluetooth usando o ID do dispositivo
        private void connectToDevice(String btAddress) {
            try {
                // A URL de conexão Bluetooth com o dispositivo usando seu endereço
                String url = "btspp://" + btAddress + ":1";  // "1" indica o perfil de porta serial (SPP)
                connection = (StreamConnection) Connector.open(url);  // Cria a conexão com o dispositivo

                System.out.println("Conectado ao dispositivo Bluetooth: " + btAddress);
                echoCommand("Bluetooth Conectado ao dispositivo com o ID: " + btAddress);

                // Após conectar, você pode enviar ou receber dados
                InputStream in = connection.openInputStream();
                OutputStream out = connection.openOutputStream();

                // Exemplo de como enviar dados
                out.write("Conectado ao dispositivo Bluetooth.".getBytes());
                out.flush();
                echoCommand("Dados enviados para o dispositivo.");

            } catch (IOException e) {
                System.out.println("Erro ao conectar ao dispositivo Bluetooth: " + e.getMessage());
                echoCommand("Erro ao conectar ao dispositivo Bluetooth.");
            }
        }

        // Comando para verificar o status da conexão
        private void checkStatus() {
            if (connection != null) {
                echoCommand("Conexão Bluetooth está ativa.");
            } else {
                echoCommand("Nenhuma conexão Bluetooth ativa.");
            }
        }

        // Comando para enviar dados ao dispositivo Bluetooth
        private void sendData(String data) {
            if (connection != null) {
                try {
                    ...
                    out.flush();
                    echoCommand("Dados enviados: " + data);
                } catch (IOException e) {
                    echoCommand("Erro ao enviar dados: " + e.getMessage());
                }
            } else {
                echoCommand("Nenhuma conexão Bluetooth ativa para enviar dados.");
            }
        }
