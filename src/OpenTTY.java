...
public class OpenTTY extends MIDlet implements CommandListener {
    ...
    private void ticker(String text) { Displayable current = display.getCurrent(); if (text == null || text.length() == 0) { current.setTicker(null); } else { current.setTicker(new Ticker(text)); } }
    ...
    public class PortScanner implements CommandListener, Runnable { private List ports = new List(host + " Ports", List.IMPLICIT); private String host; public PortScanner(String args) { if (args == null || args.length() == 0) { return; } host = args; ports.addCommand(new Command("Connect", Command.OK, 1)); ports.addCommand(new Command("Back", Command.BACK, 2)); ports.setCommandListener(this); new Thread(this).start(); display.setCurrent(ports); } public void commandAction(Command c, Displayable d) { if (c.getCommandType() == Command.OK) { new RemoteConnection(host + ":" + ports.getString(ports.getSelectedIndex())); } else if (c.getCommandType() == Command.BACK) { processCommand("xterm"); } } public void run() { for (int port = 1; port <= 65535; port++) { try { SocketConnection socket = (SocketConnection) Connector.open("socket://" + host + ":" + port); ports.append(Integer.toString(port), null); socket.close(); } catch (IOException e) { } } } }
