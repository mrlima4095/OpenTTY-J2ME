...
public class OpenTTY extends MIDlet implements CommandListener {
    ...
    private void processCommand(String command) { processCommand(command, true, null, cloneattr()); }
    private void processCommand(String command, boolean ignore) { processCommand(command, ignore, null, cloneattr()); }
    private void processCommand(String command, boolean ignore, Vector args) { processCommand(command, ignore, args, cloneattr()); }
    private void processCommand(String command, boolean ignore, Vector args, Hashtable scope) {
        ...
        else if (mainCommand.equals("unalias")) { if (argument.equals("")) { } else if (aliases.containsKey(argument)) { aliases.remove(argument); } else { echoCommand("unalias: " + argument + ": not found"); } }
        // |
        // Environment Keys
        ...
        else if (mainCommand.equals("sh") || mainCommand.equals("login")) { processCommand("import /java/bin/sh", false); }
        ...
        else if (mainCommand.equals("top")) { if (argument.equals("")) { new HTopViewer("monitor"); } else if (argument.equals("used")) { echoCommand("" + (runtime.totalMemory() - runtime.freeMemory()) / 1024); } else if (argument.equals("free")) { echoCommand("" + runtime.freeMemory() / 1024); } else if (argument.equals("total")) { echoCommand("" + runtime.totalMemory() / 1024); } else { echoCommand("top: " + getCommand(argument) + ": not found"); } }
        // |
        // Process
        ...
        else if (mainCommand.equals("pong")) { if (argument.equals("")) { } else { long startTime = System.currentTimeMillis(); SocketConnection socket = null; try { socket = (SocketConnection) Connector.open("socket://" + argument); long endTime = System.currentTimeMillis(); echoCommand("Pong to " + argument + " successful, time=" + (endTime - startTime) + "ms"); } catch (IOException e) { echoCommand("Pong to " + argument + " failed: " + e.getMessage()); } } }
        else if (mainCommand.equals("ping")) { if (argument.equals("")) { } else { if (!argument.startsWith("http://") && !argument.startsWith("https://")) { argument = "http://" + argument; } long startTime = System.currentTimeMillis(); try { HttpConnection conn = (HttpConnection) Connector.open(argument); conn.setRequestMethod(HttpConnection.GET); int responseCode = conn.getResponseCode(); long endTime = System.currentTimeMillis(); echoCommand("Ping to " + argument + " successful, time=" + (endTime - startTime) + "ms"); conn.close(); } catch (IOException e) { echoCommand("Ping to " + argument + " failed: " + e.getMessage()); } } }
        ...
        else if (mainCommand.equals("lsblk")) { if (argument.equals("") || argument.equals("-x")) { echoCommand(replace("MIDlet.RMS.Storage", ".", argument.equals("-x") ? ";" : "\t")); } else { echoCommand("lsblk: " + argument + ": not found"); } }
        // |
        // RMS Files
        ...
        // |
        // Permissions
        else if (mainCommand.equals("chmod")) { if (argument.equals("")) { } else { Hashtable nodes = parseProperties("http=javax.microedition.io.Connector.http\nsocket=javax.microedition.io.Connector.socket\nfile=javax.microedition.io.Connector.file\nprg=javax.microedition.io.PushRegistry"); int status = 1; if (nodes.containsKey(argument)) { try { if (argument.equals("http")) { ((HttpConnection) Connector.open("http://google.com")).close(); } else if (argument.equals("socket")) { ((SocketConnection) Connector.open(env("socket://$REPO"))).close(); } else if (argument.equals("file")) { FileSystemRegistry.listRoots(); } else if (argument.equals("prg")) { PushRegistry.registerAlarm(getClass().getName(), System.currentTimeMillis() + 1000); } } catch (SecurityException e) { status = 2; } catch (Exception e) { status = 3; } } else if (argument.equals("*")) { Enumeration keys = nodes.keys(); while (keys.hasMoreElements()) { processCommand("chmod " + (String) keys.nextElement(), false); status = 4; } } else { echoCommand("chmod: " + argument + ": not found"); status = 5; } if (status == 1) { MIDletLogs("add info Permission '" + (String) nodes.get(argument) + "' granted"); } else if (status == 2) { MIDletLogs("add error Permission '" + (String) nodes.get(argument) + "' denied"); } else if (status == 3) { MIDletLogs("add warn Unsupported API '" + (String) nodes.get(argument) + "'"); } } }
        // |
        // General Utilities
    ...
    private String env(String text, Hashtable scope) { text = replace(text, "$PATH", path); text = replace(text, "$USERNAME", username); text = replace(text, "$TITLE", form.getTitle()); text = replace(text, "$PROMPT", stdin.getString()); text = replace(text, "\\n", "\n"); text = replace(text, "\\r", "\r"); text = replace(text, "\\t", "\t"); Enumeration e = scope.keys(); while (e.hasMoreElements()) { String key = (String) e.nextElement(); String value = (String) scope.get(key); text = replace(text, "$" + key, value); } text = replace(text, "$.", "$"); text = replace(text, "\\.", "\\"); return text; }
    ...
    private Hashtable cloneAttributes() {
        Hashtable copy = new Hashtable();
        Enumeration keys = attributes.keys();
        while (keys.hasMoreElements()) {
            String key = (String) keys.nextElement();
            copy.put(key, attributes.get(key));
        }
        return copy;
    }
    
    private Vector splitToVector(String text) {
        Vector vec = new Vector();
        if (text == null || text.trim().equals("")) return vec;
        String[] parts = split(text, ' ');
        for (int i = 0; i < parts.length; i++) {
            vec.addElement(parts[i].trim());
        }
        return vec;
    }
            ...
            if (type.equals("make")) {
                ...
                if (lib.containsKey("screen.fields")) {
                    ...
                    for (int i = 0; i < fields.length; i++) {
                        ...
                        else if (type.equals("spacer")) { int width = Integer.parseInt(getenv("screen." + field + ".w", "1")), height = Integer.parseInt(getenv("screen." + field + ".h", "10")); screen.append(new Spacer(width, height)); }
                    }
                }
    ...
    private void GetAddress(String command) { command = env(command.trim()); String mainCommand = getCommand(command); String argument = getArgument(command); if (mainCommand.equals("")) { processCommand("ifconfig"); } else { try { DatagramConnection conn = (DatagramConnection) Connector.open("datagram://" + (argument.equals("") ? "1.1.1.1:53" : argument)); ByteArrayOutputStream out = new ByteArrayOutputStream(); out.write(0x12); out.write(0x34); out.write(0x01); out.write(0x00); out.write(0x00); out.write(0x01); out.write(0x00); out.write(0x00); out.write(0x00); out.write(0x00); out.write(0x00); out.write(0x00); String[] parts = split(mainCommand, '.'); for (int i = 0; i < parts.length; i++) { out.write(parts[i].length()); out.write(parts[i].getBytes()); } out.write(0x00); out.write(0x00); out.write(0x01); out.write(0x00); out.write(0x01); byte[] query = out.toByteArray(); Datagram request = conn.newDatagram(query, query.length); conn.send(request); Datagram response = conn.newDatagram(512); conn.receive(response); conn.close(); byte[] data = response.getData(); if ((data[3] & 0x0F) != 0) { echoCommand("not found"); return; } int offset = 12; while (data[offset] != 0) { offset++; } offset += 5; if (data[offset + 2] == 0x00 && data[offset + 3] == 0x01) { StringBuffer ip = new StringBuffer(); for (int i = offset + 12; i < offset + 16; i++) { ip.append(data[i] & 0xFF); if (i < offset + 15) ip.append("."); } echoCommand(ip.toString()); } else { echoCommand("not found"); } } catch (IOException e) { echoCommand(e.getMessage()); } } }
    ...
    // |
    // Audio Manager
    private void audio(String command) { command = env(command.trim()); String mainCommand = getCommand(command); String argument = getArgument(command); if (mainCommand.equals("")) { } else if (mainCommand.equals("volume")) { if (player != null) { VolumeControl vc = (VolumeControl) player.getControl("VolumeControl"); if (argument.equals("")) { echoCommand("" + vc.getLevel()); } else { try { vc.setLevel(Integer.parseInt(argument)); } catch (Exception e) { echoCommand(e.getMessage()); } } } else { echoCommand("audio: not running."); } } else if (mainCommand.equals("play")) { if (argument.equals("")) { } else { if (argument.startsWith("/mnt/")) { argument = argument.substring(5); } else if (argument.startsWith("/")) { echoCommand("audio: invalid source."); return; } else { audio("play " + path + argument); return; } try { FileConnection fc = (FileConnection) Connector.open("file:///" + argument, Connector.READ); if (!fc.exists()) { echoCommand("audio: " + basename(argument) + ": not found"); return; } InputStream is = fc.openInputStream(); player = Manager.createPlayer(is, getMimeType(argument)); player.prefetch(); player.start(); start("audio"); } catch (Exception e) { echoCommand(e.getMessage()); } } } else if (mainCommand.equals("pause")) { try { if (player != null) { player.stop(); } else { echoCommand("audio: not running."); } } catch (Exception e) { echoCommand(e.getMessage()); } } else if (mainCommand.equals("resume")) { try { if (player != null) { player.start(); } else { echoCommand("audio: not running."); } } catch (Exception e) { echoCommand(e.getMessage()); } } else if (mainCommand.equals("stop")) { try { if (player != null) { player.stop(); player.close(); player = null; stop("audio"); } } catch (Exception e) { echoCommand(e.getMessage()); } } else { echoCommand("audio: " + mainCommand + ": not found"); } } private String getMimeType(String filename) { filename = filename.toLowerCase(); if (filename.endsWith(".amr")) { return "audio/amr"; } else if (filename.endsWith(".wav")) { return "audio/x-wav"; } else { return "audio/mpeg"; } }
