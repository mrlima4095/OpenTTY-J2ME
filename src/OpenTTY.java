...
public class OpenTTY extends MIDlet implements CommandListener {
    ...
    private int writeRMS(String filename, byte[] data, int index) { if (filename == null || filename.length() == 0 || data == null) { return 2; }  RecordStore CONN = null; try { CONN = RecordStore.openRecordStore(filename, true); while (CONN.getNumRecords() < index) { CONN.addRecord("".getBytes(), 0, 0); } CONN.setRecord(index, data, 0, data.length); } catch (RecordStoreException e) { return 1; } finally { if (CONN != null) { try { CONN.closeRecordStore(); } catch (RecordStoreException e) { } } } return 0; }
