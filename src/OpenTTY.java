...
public class OpenTTY extends MIDlet implements CommandListener {
    ...
    // |
    private String getcontent(String file) { file = getpattern(file); return file.startsWith("/") ? read(file) : file.equals("nano") ? nanoContent : read(path + file); }
    ...
    private int xserver(String command, boolean root) {
        ...
        else if (mainCommand.equals("stop")) { form.setTitle(null); form.setTicker(null); form.deleteAll(); xserver("cmd hide", root); desktops = new Hashtable(); form.removeCommand(EXECUTE); }
