...
public class OpenTTY extends MIDlet implements CommandListener {
    ...
    private String logs = "", path = "/", build = "2025-1.14.2-01x80";
    ...
    private void chmod(String node) { if (node == null || node.length() == 0) { return; } Hashtable nodes = parseProperties(read("/java/etc/perms.ini")); int status = 1; if (nodes.containsKey(node)) { try { if (node.equals("http") || node.equals("socket") || node.equals("datagram")) { ((StreamConnection) Connector.open(env(node + "://$REPO"))).close(); } else if (node.equals("prg")) { PushRegistry.registerAlarm(getClass().getName(), System.currentTimeMillis() + 1000); } else if (node.equals("file")) { FileSystemRegistry.listRoots(); } } catch (SecurityException e) { status = 2; } catch (Exception e) { echoCommand("chmod: " + e.getMessage()); return; } } else { echoCommand("chmod: " + node + ": not found"); return; } if (status == 1) MIDletLogs("add info Permission '" + (String) nodes.get(node) + "' granted"); else if (status == 2) MIDletLogs("add error Permission '" + (String) nodes.get(node) + "' denied"); }
