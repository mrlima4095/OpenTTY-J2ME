...
public class OpenTTY extends MIDlet implements CommandListener {
    ...
    private String passwd(boolean write, String value) {
        if (write) { value = value == null ? "" : value; writeRMS("OpenRMS", String.valueOf(value.hashCode()).getBytes(), 2); }
        else {
            try { 
    ...
    private int writeRMS(String filename, byte[] data, int index) { if (filename == null || filename.length() == 0 || index == null || data == null) { return 2; }  RecordStore CONN = null; try { CONN = RecordStore.openRecordStore(filename, true); while (CONN.getNumRecords() < index) { CONN.addRecord("".getBytes(), 0, 0); } CONN.setRecord(index, data, 0, data.length); } catch (RecordStoreException e) { return 1; } finally { if (CONN != null) { try { CONN.closeRecordStore(); } catch (RecordStoreException e) { } } } return 0; }
