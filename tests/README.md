...
## Quick start
    ...
    ./tests/run_all.sh              # default 5 suites
...
| Desktop kernel | `tests/python/test_desktop.py` | boots the Python port of the kernel (`lua/sys`) headless in an isolated devroot and drives real shell commands (`uname`, `ls`, `cat`, `expr`, `pkg version`, `memelf`, ...). |
...
- `python3` (`check_sources.py`, `check_elves.py`, `test_desktop.py`)
