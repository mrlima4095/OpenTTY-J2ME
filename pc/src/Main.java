...
public class OpenTTY {
            ...
            while (true) {
                try {
                    ...
                    if (sys.containsKey("1")) {
                        Process proc = sys.get("1");
                        Hashtable<String, Object> args = new Hashtable<>();
                        args.put("0", "sh");
                        args.put("1", "-c");
                        args.put("2", line);
                        
                        Object result = proc.lua.internals(new Vector<>(Arrays.asList(
                            Lua.EXEC, args
                        )));
    ...
    public int write(String filename, String data, int id, Hashtable<String, String> scope) {
        return write(filename, data.getBytes(), id, scope);
    }
    ...
    private Path resolvePath(String path, Hashtable<String, String> scope) {
        String pwd = scope.getOrDefault("PWD", "/home/");
        
        if (path.startsWith("/")) {
            return rootPath.resolve(path.substring(1));
        } else {
            return rootPath.resolve(pwd.substring(1)).resolve(path);
        }
    }
    ...
    public String getArgument(String text) {
        int spaceIndex = text.indexOf(' ');
        return spaceIndex == -1 ? "" : text.substring(spaceIndex + 1).trim();
    }
    ...
    public int getUserID(String user) {
        if (user.equals("root")) return 0;
        if (user.equals(username)) return 1000;
        return userID.getOrDefault(user, -1);
    }
    
    public String getUser(int uid) {
        if (uid == 0) return "root";
        if (uid == 1000) return username;
        for (Map.Entry<String, Integer> entry : userID.entrySet()) {
            if (entry.getValue() == uid) return entry.getKey();
        }
        return null;
    }
}
