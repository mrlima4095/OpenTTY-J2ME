...
public class OpenTTY extends MIDlet implements CommandListener {
    ...
    public Command EXECUTE = new Command("Send", Command.OK, 0), HELP = new Command("Help", Command.SCREEN, 1), NANO = new Command("Nano", Command.SCREEN, 2), CLEAR = new Command("Clear", Command.SCREEN, 3), HISTORY = new Command("History", Command.SCREEN, 4), BACK = new Command("Back", Command.BACK, 1), RUNS = new Command("Run Script", Command.OK, 1), VIEW = new Command("View as HTML", Command.OK, 1);
    // |
    // MIDlet Loader
    ...
    // |
    // | (Triggers)
    public void pauseApp() { processCommand(functions.containsKey("pauseApp()") ? "pauseApp()" : "true", true, 1000, pid, stdout, scope); }
    public void destroyApp(boolean unconditional) { writeRMS("/home/nano", nanoContent, 0); notifyDestroyed(); }
    // |
    // | (Main Listener)
    ...
    public class MIDletControl implements ItemCommandListener, CommandListener, Runnable {
        ...
        public MIDletControl(String command, int id) {
        ...
        public MIDletControl(String mode, String args, int id) {
            ...
            Hashtable proc = genpr ocess(MOD == NC ? "remote" : MOD == PRSCAN ? "prscan" : "gobuster", id, null);
        ...
        public void commandAction(Command c, Displayable d) {
            ...
            if (MOD == HISTORY) { String selected = preview.getString(preview.getSelectedIndex()); if (selected != null) { processCommand("xterm"); processCommand(c == RUN || c == List.SELECT_COMMAND ? selected : "buff " + selected, true, id); } } 
                ...
                else if (c == BACK) { writeRMS("/home/remote", console.getText(), 0); back(); } 
                ...
                if (c == RUN || c == List.SELECT_COMMAND) { 
                    if (MOD == QUEST) { 
                        ...
                        if (index >= 0) { 
                            processCommand("xterm", true, id); 
                            String key = env(preview.getString(index));
                            if (PKG.containsKey("list.default")) { processCommand(getenv("list.default"), true, id); }
                            processCommand(getvalue(key, "log add warn An error occurred, '" + key + "' not found"), true, id); 
                        } 
                    } 
        ...
        private void reload() { if (attributes.containsKey("J2EMU")) { new MIDletControl(MOD == MONITOR ? "monitor" : MOD == PROCESS ? "process" : MOD == EXPLORER ? "dir" : "history", id); } else { load(); } }
        private void load() {
    ...
    // |
    // MIDlet Shell
    public int processCommand(String command, boolean ignore, String id, String pid, String stdout, Hashtable scope) { 
        ...
        // |
        // (Calls)
        else if (shell.containsKey(mainCommand) && ignore) { Hashtable args = (Hashtable) shell.get(mainCommand); if (argument.equals("")) { return processCommand(aliases.containsKey(mainCommand) ? (String) aliases.get(mainCommand) : "true", ignore, id, pid, stdout, scope); } else if (args.containsKey(getCommand(argument).toLowerCase())) { return processCommand((String) args.get(getCommand(argument)) + " " + getArgument(argument), ignore, id, pid, stdout, scope); } else { return processCommand(args.containsKey("shell.unknown") ? (String) args.get(getCommand("shell.unknown")) + " " + getArgument(argument) : "echo " + mainCommand + ": " + getCommand(argument) + ": not found", args.containsKey("shell.unknown") ? true : false, id, pid, stdout, scope); } }
        else if (aliases.containsKey(mainCommand) && ignore) { return processCommand((String) aliases.get(mainCommand) + " " + argument, ignore, id, pid, stdout, scope); }
        else if (functions.containsKey(mainCommand) && ignore) { return runScript((String) functions.get(mainCommand)); }
        else if (classpath && file("/bin/" + mainCommand) && !mainCommand.equals("sh") && !mainCommand.equals("lua") && !mainCommand.startsWith(".")) { return processCommand(". /bin/" + command, ignore, id, pid, stdout, scope); }
        // |
        // Aliases
        ...
        //String INFO = argument.equals("") || argument.equals("-i") ? "$TYPE" : argument.equals("-a") || argument.equals("--all") ? "$TYPE (OpenTTY $VERSION) main/$RELEASE " + build + " - $CONFIG $PROFILE" : argument.equals("-r") || argument.equals("--release") ? "$VERSION" : argument.equals("-v") || argument.equals("--build") ? build : argument.equals("-s") ? "J2ME" : argument.equals("-m") ? "$PROFILE" : argument.equals("-p") ? "$CONFIG" : argument.equals("-n") ? "$HOSTNAME" : null; if (INFO == null) { echoCommand("uname: " + argument + ": not found"); return 127; } else { echoCommand(env(INFO)); } else if (mainCommand.equals("uname")) { String INFO = argument.equals("") || argument.equals("-i") ? "$TYPE" : argument.equals("-a") || argument.equals("--all") ? "$TYPE (OpenTTY $VERSION) main/$RELEASE " + build + " - $CONFIG $PROFILE" : argument.equals("-r") || argument.equals("--release") ? "$VERSION" : argument.equals("-v") || argument.equals("--build") ? build : argument.equals("-s") ? "J2ME" : argument.equals("-m") ? "$PROFILE" : argument.equals("-p") ? "$CONFIG" : argument.equals("-n") ? "$HOSTNAME" : null; if (INFO == null) { echoCommand("uname: " + argument + ": not found"); return 127; } else { echoCommand(env(INFO)); } }
        // |
        // Device ID
        //else if (mainCommand.equals("hostname")) { processCommand(argument.equals("") ? "echo $HOSTNAME" : "set HOSTNAME=" + getCommand(argument), false, id, pid, stdout, scope); }
        ...
        //else if (mainCommand.equals("mmspt") || mainCommand.equals("chrt")) { if (argument.equals("")) { echoCommand(getThreadName(Thread.currentThread())); } else if (argument.equals("priority")) { echoCommand("" + Thread.currentThread().getPriority()); } else { int value = getNumber(argument, Thread.NORM_PRIORITY, true); if (value > 10 || value < 1) { return 2; } else { Thread.currentThread().setPriority(value); } } }
        //else if (mainCommand.equals("bg")) { if (argument.equals("")) { } else { new MIDletControl("Background", argument, ignore, id); } }
        ...
        // |
        //else if (mainCommand.equals("report")) { return processCommand("open mailto:felipebr4095@gmail.com", false, id, pid, stdout, scope); }
        ...
        //else if (mainCommand.equals("dir")) { new MIDletControl("dir", id); }
        ...
        /*else if (mainCommand.equals("rm")) { if (argument.equals("")) { } else { for (int i = 0; i < args.length; i++) { int STATUS = deleteFile(args[i], id); if (STATUS != 0) { return STATUS; } } } }
        else if (mainCommand.equals("install")) { if (argument.equals("")) { } else { return writeRMS(argument, nanoContent, id); } }
        else if (mainCommand.equals("touch")) { if (argument.equals("")) { nanoContent = ""; } else { for (int i = 0; i < args.length; i++) { int STATUS = writeRMS(argument, "", id); if (STATUS != 0) { return STATUS; } } } }
            ...
            if (argument.equals("")) { echoCommand("cp: missing [origin]"); } 
                ...
                try {
                    ...
                    return writeRMS(target.startsWith("/") ? target : path + target, buffer.toByteArray(), id);
            ...
            else if (args[0].equals("read")) { if (args.length < 2) { return 2; } else { echoCommand(loadRMS("OpenRMS", args[1].equals("/bin/") ? 3 : args[1].equals("/etc/") ? 5 : 4)); } }
        ...
        //else if (mainCommand.equals("du")) { if (argument.equals("")) { } else { processCommand("wc -c " + argument, false, id, pid, stdout, scope); } }
...
//        // |
        ...
        //else if (mainCommand.equals("help")) { viewer(form.getTitle(), read("/res/docs")); }
            ...
            String content = loadRMS("man.html"); 
            if (content.equals("") || argument.equals("--update")) { 
                int STATUS = processCommand("netstat ipinfo.io", false, pid, stdout, scope); 
                if (STATUS == 0) { 
                    STATUS = processCommand("execute install /home/nano; tick Downloading...; proxy github.com/mrlima4095/OpenTTY-J2ME/raw/refs/heads/main/assets/root/man.html; install /home/man.html; get; tick;", false, pid, stdout, scope); 
        ...
        //else if (mainCommand.equals("which")) { if (argument.equals("")) { } else { echoCommand(shell.containsKey(argument) ? "shell" : (aliases.containsKey(argument) ? "alias" : (functions.containsKey(argument) ? "function" : file("/bin/" + mainCommand) ? "application" : ""))); } }
        ...
        //else if (mainCommand.equals("@reload")) { aliases = new Hashtable(); shell = new Hashtable(); functions = new Hashtable(); username = loadRMS("OpenRMS"); processCommand("execute log add debug API reloaded; sh; x11 stop; x11 init; . /home/.initrc", true, id, pid, stdout, scope); } 
        ...
        //else if (mainCommand.equals("import")) { return importScript(getcontent(argument), id); }
        ...
        //else if (mainCommand.equals("eval")) { if (argument.equals("")) { } else { echoCommand("" + processCommand(argument, ignore, id, pid, stdout, scope)); } }
        //else if (mainCommand.equals("catch")) { if (argument.equals("")) { } else { try { processCommand(argument, ignore, id, pid, stdout, scope); } catch (Throwable e) { echoCommand(getCatch(e)); } } }
    ...
    // |
    public String getCommand(String input) { int spaceIndex = input.indexOf(' '); if (spaceIndex == -1) { return input; } else { return input.substring(0, spaceIndex); } }
    public String getArgument(String input) { int spaceIndex = input.indexOf(' '); if (spaceIndex == -1) { return ""; } else { return input.substring(spaceIndex + 1).trim(); } }
    // |
    // Readers
    ...
    // |
    // OpenTTY Logging Manager
    public int MIDletLogs(String command) { command = env(command.trim()); String mainCommand = getCommand(command), argument = getArgument(command); if (mainCommand.equals("")) { } else if (mainCommand.equals("clear")) { logs = ""; } else if (mainCommand.equals("swap")) { writeRMS(argument.equals("") ? "logs" : argument, logs, 0); } else if (mainCommand.equals("view")) { viewer(form.getTitle(), logs); } else if (mainCommand.equals("add")) { String LEVEL = getCommand(argument).toLowerCase(), MESSAGE = getArgument(argument); if (!MESSAGE.equals("")) { if (LEVEL.equals("info") || LEVEL.equals("warn") || LEVEL.equals("debug") || LEVEL.equals("error")) { logs += "[" + LEVEL.toUpperCase() + "] " + split(new java.util.Date().toString(), ' ')[3] + " " + MESSAGE + "\n"; } else { echoCommand("log: add: " + LEVEL + ": not found"); return 127; } } } else { echoCommand("log: " + mainCommand + ": not found"); return 127; } return 0; }

    // API 004 - (LCDUI Interface)
    // |
    // System UI
    public int xserver(String command, int id, String pid, String stdout, Hashtable scope) {
        ...
        else if (mainCommand.equals("init")) { start("x11-wm", null, null, id); } 
        else if (mainCommand.equals("stop")) { form.setTitle(""); form.setTicker(null); form.deleteAll(); xserver("cmd hide", id); form.removeCommand(EXECUTE); trace.remove("2"); }
        else if (mainCommand.equals("xfinit")) { if (argument.equals("")) { return xserver("init", id); } if (argument.equals("stdin")) { form.append(stdin); } else if (argument.equals("stdout")) { form.append(stdout); } }
        ...
        // Screen Manager
            ...
            if (argument.equals("")) { } 
                ...
                if (trace.containsKey(argument)) {
                    if (!getowner(argument).equals(username) && id != 0) { echoCommand("Permission denied!"); return 13; }
        ...
        // |
        // Interfaces
        else if (mainCommand.equals("canvas")) { if (javaClass("MIDletCanvas") == 0) { display.setCurrent(new MIDletCanvas(this, argument.equals("") ? "Canvas" : argument.startsWith("-e") ? argument.substring(2).trim() : getcontent(argument), id)); } else { echoCommand("This MIDlet Build don't have MIDletCanvas"); return 3; } }
        else if (mainCommand.equals("make") || mainCommand.equals("list") || mainCommand.equals("quest") || mainCommand.equals("edit")) { new MIDletControl(mainCommand.equals("make") ? MIDletControl.SCREEN : mainCommand.equals("list") ? MIDletControl.LIST : mainCommand.equals("quest") ? MIDletControl.QUEST : MIDletControl.WEDIT, argument.startsWith("-e") ? argument.substring(2).trim() : getcontent(argument), id); }
        else if (mainCommand.equals("item")) { if (argument.equals("") || argument.equals("clear")) { form.deleteAll(); form.append(stdout); form.append(stdin); } else { new MIDletControl(form, "item", argument.startsWith("-e") ? argument.substring(2).trim() : getcontent(argument), id); } }
    ...
    // |
    public int warnCommand(String title, String message) { if (message == null || message.length() == 0) { return 2; } Alert alert = new Alert(title, message, null, AlertType.WARNING); alert.setTimeout(Alert.FOREVER); display.setCurrent(alert); return 0; }
    ...
    private int caseCommand(String argument, boolean ignore, int id, String pid, String stdout, Hashtable scope) { 
        ...
        if (CONDITION != NEGATED) { return processCommand(CMD, ignore, id, pid, stdout, scope); } 
    ...
    public int kill(String pid, boolean print, int id, String stdout, Hashtable scope) {
        ...
        if (!owner.equals(username) && id != 0) { if (print) { echoCommand("Permission denied!"); } return 13; }
    ...
    public int start(String app, String pid, String collector, int id, String stdout, Hashtable scope) {
        ...
        if (app.equals("sh") || app.equals("x11-wm")) {
            ...
            if (trace.containsKey(pid)) { return 68; }
                ...
                processCommand("execute title; x11 cmd;", false, id, pid, stdout, scope); 
    ...
    public int stop(String app, int id, String pid, String stdout, Hashtable scope) { 
        if (app == null || app.length() == 0) { return 2; } 
        int STATUS = 0; 
        ...
        for (Enumeration keys = trace.keys(); keys.hasMoreElements();) { 
            ...
            if (app.equals(NAME)) { if ((STATUS = kill(PID, false, id, stdout, scope)) != 0) { break; } } } return STATUS; }
    // | 
    // Kernel
    ...
    public String getsignal(String pid, String signal) { if (trace.containsKey(pid)) { Hashtable signals = getobject(pid, "signals"); if (signals != null && signals.containsKey(signal)) { return signals.get(signal).toString(); } else { return null; } } else { return null; } } 
    ...
    // |
    // Text related commands
    public void echoCommand(String message) { echoCommand(message, stdout); attributes.put("OUTPUT", message); }
    public void echoCommand(String message, StringItem console) { 
        if (message == null) { return; } 
        String current = console.getText(), 
            output = current == null || current.length() == 0 ? message : current + "\n" + message; 
        console.setText(TTY_MAX_LEN >= 0 && output.length() > TTY_MAX_LEN ? output.substring(output.length() - TTY_MAX_LEN) : output); }
    ...
    private String diirname(String path) { if (path == null || path.length() == 0) { return ""; } if (path.endsWith("/")) { path = path.substring(0, path.length() - 1); } int lastSlashIndex = path.lastIndexOf('/'); if (lastSlashIndex == -1) { return path; } return path.substring(0, lastSlashIndex + 1); }
    ...
    public int deleteFile(String filename, int id) { 
        if (filename == null || filename.length() == 0) { return 2; } 
            ...
            if (content.indexOf("[\0BEGIN:" + name + "\0]") == -1) { echoCommand("read-only storage"); return 5; }

            if (useCache) { cache.remove("/" + base + "/" + name); }
            return writeRMS("OpenRMS", delFile(name, content).getBytes(), index, id);
        }
    ...
    public int writeRMS(String filename, byte[] data, int id) { 
        if (filename == null || filename.length() == 0) { return 2; } 
            ...
            else if (filename.equals("sh") || filename.equals("lua")) { echoCommand("read-only storage"); return 5; }
        ...
        else if (filename.startsWith("/dev/")) { if ((filename = filename.substring(5)).equals("")) { return 2; } else if (filename.equals("null")) { } else if (filename.equals("stdin")) { stdin.setString(new String(data)); } else if (filename.equals("stdout")) { stdout.setText(new String(data)); } else { echoCommand("read-only storage"); return 5; } }
        else if (filename.startsWith("/tmp/")) { if ((filename = filename.substring(5)).equals("")) { return 2; } else { tmp.put(filename, new String(data)); } }
        else if (filename.startsWith("/")) { echoCommand("read-only storage"); return 5; } 
        else { return writeRMS(path + filename, data, id); } return 0; }
    ...
    public int java(String command) {
        ...
        else if (mainCommand.equals("--version")) { echoCommand(getName()); }
    ...
    private int importScript(String script, int id) {
        ...
        if (PKG.containsKey("api.version")) {
            ...
            if (fail) { String error = (String) PKG.get("api.error"); processCommand(error != null ? error : "true", true, id, pid, stdout, scope); return 3; }
        }
        ...
        // |
        // Start Application
        if (PKG.containsKey("config")) { int STATUS = processCommand((String) PKG.get("config"), true, id, pid, stdout, scope); if (STATUS != 0) { return STATUS; } }
...
public class OpenTTY extends MIDlet implements CommandListener {
    ...
    public int processCommand(String command, boolean ignore, int id) { 
        ...
        else if (classpath && file("/bin/" + mainCommand) && !mainCommand.equals("sh") && !mainCommand.equals("lua") && !mainCommand.startsWith(".")) { return processCommand(". /bin/" + command, ignore, id); }
        // |
        // Aliases
        else if (mainCommand.equals("alias")) { if (argument.equals("")) { for (Enumeration KEYS = aliases.keys(); KEYS.hasMoreElements();) { String KEY = (String) KEYS.nextElement(), VALUE = (String) aliases.get(KEY); if (!KEY.equals("xterm") && !VALUE.equals("")) { echoCommand("alias " + KEY + "='" + VALUE.trim() + "'"); } } } else { int INDEX = argument.indexOf('='); if (INDEX == -1) { for (int i = 0; i < args.length; i++) { if (aliases.containsKey(args[i])) { echoCommand("alias " + args[i] + "='" + (String) aliases.get(args[i]) + "'"); } else { echoCommand("alias: " + argument + ": not found"); return 127; } } } else { aliases.put(argument.substring(0, INDEX).trim(), getpattern(argument.substring(INDEX + 1).trim())); } } }  
        else if (mainCommand.equals("unalias")) { if (argument.equals("")) { } else { for (int i = 0; i < args.length; i++) { if (aliases.containsKey(args[i])) { aliases.remove(args[i]); } else { echoCommand("unalias: "+ args[i] + ": not found"); return 127; } } } }
        ...
        else if (mainCommand.equals("logout")) { if (loadRMS("OpenRMS").equals(username)) { if (id == 0) { writeRMS("/home/OpenRMS", "", id); processCommand("exit", false); } else { echoCommand("Permission denied!"); return 13; } } else { username = loadRMS("OpenRMS"); processCommand("sh", false); } }
        else if (mainCommand.equals("exit") || mainCommand.equals("quit")) { if (loadRMS("OpenRMS").equals(username)) { destroyApp(false); } else { username = loadRMS("OpenRMS"); processCommand("sh", false); } }
        ...
        // |
        // Long executors
        else if (mainCommand.equals("builtin") || mainCommand.equals("command")) { return processCommand(argument, false, id); }
        else if (mainCommand.equals("bruteforce")) { String PID = genpid(); start("bruteforce", PID, null, id); while (trace.containsKey(PID)) { int STATUS = processCommand(argument, ignore, id); if (STATUS != 0) { kill(PID, false, id); return STATUS; } } }
        else if (mainCommand.equals("cron")) { if (argument.equals("")) { } else { return processCommand("execute sleep " + getCommand(argument) + "; " + getArgument(argument), ignore, id); } }
        else if (mainCommand.equals("sleep")) { if (argument.equals("")) { } else { try { Thread.sleep(Integer.parseInt(argument) * 1000); } catch (Exception e) { echoCommand(getCatch(e)); return 2; } } }
        else if (mainCommand.equals("time")) { if (argument.equals("")) { } else { long START = System.currentTimeMillis(); int STATUS = processCommand(argument, ignore, id); echoCommand("at " + (System.currentTimeMillis() - START) + "ms"); return STATUS; } }
        // |
        // Chain executors
        else if (mainCommand.startsWith("exec")) { String[] CMDS = split(argument, mainCommand.equals("exec") ? '&' : ';'); for (int i = 0; i < CMDS.length; i++) { int STATUS = processCommand(CMDS[i].trim(), ignore, id); if (STATUS != 0) { return STATUS; } } }
        ...
        // |
        // Device Files
        else if (mainCommand.equals("fdisk")) { return processCommand("ls /mnt/", false, id); }
        else if (mainCommand.equals("lsblk")) { if (argument.equals("") || argument.equals("-x")) { echoCommand(replace("MIDlet.RMS.Storage", ".", argument.equals("-x") ? ";" : "\t")); } else { echoCommand("lsblk: " + argument + ": not found"); return 127; } }
        // |
        // RMS Files
        else if (mainCommand.equals("rm")) { if (argument.equals("")) { } else { for (int i = 0; i < args.length; i++) { int STATUS = deleteFile(args[i], id); if (STATUS != 0) { return STATUS; } } } }
        ...
        // Permissions
            ...
            if (argument.equals("")) { } 
            else if (argument.equals("*")) { return processCommand("chmod http socket file prg", false, id); }
            else { 
                Hashtable NODES = parseProperties("http=javax.microedition.io.Connector.http\nsocket=javax.microedition.io.Connector.socket\nfile=javax.microedition.io.Connector.file\nprg=javax.microedition.io.PushRegistry"); 
        ...
        // General Utilities
            ...
            String content = loadRMS("man.html"); 
            if (content.equals("") || argument.equals("--update")) { 
                int STATUS = processCommand("netstat", false); 
                if (STATUS == 0) { 
        ...
        else if (mainCommand.equals("eval")) { if (argument.equals("")) { } else { echoCommand("" + processCommand(argument, ignore, id)); } }
        else if (mainCommand.equals("catch")) { if (argument.equals("")) { } else { try { processCommand(argument, ignore, id); } catch (Throwable e) { echoCommand(getCatch(e)); } } }
        else if (mainCommand.equals("return")) { return getNumber(argument, 2, true); }
    ...
    // |
    // System UI
    public int xserver(String command, int id) {
        command = env(command.trim());
        String mainCommand = getCommand(command), argument = getArgument(command);
        String[] args = splitArgs(getArgument(command));
    ...
    // |
    // Operators
    private int ifCommand(String argument, boolean ignore, int id) { argument = argument.trim(); int firstParenthesis = argument.indexOf('('), lastParenthesis = argument.indexOf(')'); if (firstParenthesis == -1 || lastParenthesis == -1 || firstParenthesis > lastParenthesis) { echoCommand("if (expr) [command]"); return 2; } String EXPR = argument.substring(firstParenthesis + 1, lastParenthesis).trim(), CMD = argument.substring(lastParenthesis + 1).trim(); String[] PARTS = split(EXPR, ' '); if (PARTS.length == 3) { boolean CONDITION = false; boolean NEGATED = PARTS[1].startsWith("!") && !PARTS[1].equals("!="); if (NEGATED) { PARTS[1] = PARTS[1].substring(1); } Double N1 = getNumber(PARTS[0]), N2 = getNumber(PARTS[2]); if (N1 != null && N2 != null) { if (PARTS[1].equals("==")) { CONDITION = N1.doubleValue() == N2.doubleValue(); } else if (PARTS[1].equals("!=")) { CONDITION = N1.doubleValue() != N2.doubleValue(); } else if (PARTS[1].equals(">")) { CONDITION = N1.doubleValue() > N2.doubleValue(); } else if (PARTS[1].equals("<")) { CONDITION = N1.doubleValue() < N2.doubleValue(); } else if (PARTS[1].equals(">=")) { CONDITION = N1.doubleValue() >= N2.doubleValue(); } else if (PARTS[1].equals("<=")) { CONDITION = N1.doubleValue() <= N2.doubleValue(); } } else { if (PARTS[1].equals("startswith")) { CONDITION = PARTS[0].startsWith(PARTS[2]); } else if (PARTS[1].equals("endswith")) { CONDITION = PARTS[0].endsWith(PARTS[2]); } else if (PARTS[1].equals("contains")) { CONDITION = PARTS[0].indexOf(PARTS[2]) != -1; } else if (PARTS[1].equals("==")) { CONDITION = PARTS[0].equals(PARTS[2]); } else if (PARTS[1].equals("!=")) { CONDITION = !PARTS[0].equals(PARTS[2]); } } if (CONDITION != NEGATED) { return processCommand(CMD, ignore, id); } } else if (PARTS.length == 2) { if (PARTS[0].equals(PARTS[1])) { return processCommand(CMD, ignore, id); } } else if (PARTS.length == 1) { if (!PARTS[0].equals("")) { return processCommand(CMD, ignore, id); } } return 0; }
    private int forCommand(String argument, boolean ignore, int id) { argument = argument.trim(); int firstParenthesis = argument.indexOf('('), lastParenthesis = argument.indexOf(')'); if (firstParenthesis == -1 || lastParenthesis == -1 || firstParenthesis > lastParenthesis) { return 2; } String KEY = getCommand(argument), FILE = getcontent(argument.substring(firstParenthesis + 1, lastParenthesis).trim()), CMD = argument.substring(lastParenthesis + 1).trim(); if (KEY.startsWith("(")) { return 2; } if (KEY.startsWith("$")) { KEY = replace(KEY, "$", ""); } String[] LINES = split(FILE, '\n'); for (int i = 0; i < LINES.length; i++) { if (LINES[i] != null || LINES[i].length() != 0) { processCommand("set " + KEY + "=" + LINES[i], false, id); int STATUS = processCommand(CMD, ignore, id); processCommand("unset " + KEY, false, id); if (STATUS != 0) { return STATUS; } } } return 0; }
    private int caseCommand(String argument, boolean ignore, int id) { 
        argument = argument.trim(); 
        int firstParenthesis = argument.indexOf('('), lastParenthesis = argument.indexOf(')'); 
        if (firstParenthesis == -1 || lastParenthesis == -1 || firstParenthesis > lastParenthesis) { return 2; } 
    ...
    public int kill(String pid, boolean print, int id) {
        ...
        if (!owner.equals(username) && id != 0) { if (print) { echoCommand("Permission denied!"); } return 13; }
        if (collector != null && collector instanceof String) { processCommand((String) collector, true, id); }
    ...
    // Text related commands
    public void echoCommand(String message) { echoCommand(message, stdout); attributes.put("OUTPUT", message); }
    public void echoCommand(String message, StringItem console) { if (message == null) { return; } String current = console.getText(), output = current == null || current.length() == 0 ? message : current + "\n" + message; console.setText(TTY_MAX_LEN >= 0 && output.length() > TTY_MAX_LEN ? output.substring(output.length() - TTY_MAX_LEN) : output); }
    private String basename(String path) { if (path == null || path.length() == 0) { return ""; } if (path.endsWith("/")) { path = path.substring(0, path.length() - 1); } int lastSlashIndex = path.lastIndexOf('/'); if (lastSlashIndex == -1) { return path; } return path.substring(lastSlashIndex + 1); }
    private String diirname(String path) { if (path == null || path.length() == 0) { return ""; } if (path.endsWith("/")) { path = path.substring(0, path.length() - 1); } int lastSlashIndex = path.lastIndexOf('/'); if (lastSlashIndex == -1) { return path; } return path.substring(0, lastSlashIndex + 1); }
    private String exprCommand(String expr) { char[] tokens = expr.toCharArray(); double[] vals = new double[32]; char[] ops = new char[32]; int valTop = -1, opTop = -1; int i = 0, len = tokens.length; while (i < len) { char c = tokens[i]; if (c == ' ') { i++; continue; } if (c >= '0' && c <= '9') { double num = 0; while (i < len && tokens[i] >= '0' && tokens[i] <= '9') { num = num * 10 + (tokens[i++] - '0'); } if (i < len && tokens[i] == '.') { i++; double frac = 0, div = 10; while (i < len && tokens[i] >= '0' && tokens[i] <= '9') { frac += (tokens[i++] - '0') / div; div *= 10; } num += frac; } vals[++valTop] = num; } else if (c == '(') { ops[++opTop] = c; i++; } else if (c == ')') { while (opTop >= 0 && ops[opTop] != '(') { double b = vals[valTop--], a = vals[valTop--]; char op = ops[opTop--]; vals[++valTop] = applyOpSimple(op, a, b); } opTop--; i++; } else if (c == '+' || c == '-' || c == '*' || c == '/') { while (opTop >= 0 && prec(ops[opTop]) >= prec(c)) { double b = vals[valTop--], a = vals[valTop--]; char op = ops[opTop--]; vals[++valTop] = applyOpSimple(op, a, b); } ops[++opTop] = c; i++; } else { return "expr: invalid char '" + c + "'"; } } while (opTop >= 0) { double b = vals[valTop--], a = vals[valTop--]; char op = ops[opTop--]; vals[++valTop] = applyOpSimple(op, a, b); } double result = vals[valTop]; return ((int) result == result) ? String.valueOf((int) result) : String.valueOf(result); } private int prec(char op) { if (op == '+' || op == '-') return 1; if (op == '*' || op == '/') return 2; return 0; } private double applyOpSimple(char op, double a, double b) { if (op == '+') return a + b; if (op == '-') return a - b; if (op == '*') return a * b; if (op == '/') return b == 0 ? 0 : a / b; return 0; }
    private String generateUUID() { String chars = "0123456789abcdef"; StringBuffer uuid = new StringBuffer(); for (int i = 0; i < 36; i++) { if (i == 8 || i == 13 || i == 18 || i == 23) { uuid.append('-'); } else if (i == 14) { uuid.append('4'); } else if (i == 19) { uuid.append(chars.charAt(8 + random.nextInt(4))); } else { uuid.append(chars.charAt(random.nextInt(16))); } } return uuid.toString(); }
