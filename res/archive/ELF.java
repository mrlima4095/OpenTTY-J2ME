...
public class ELF {
    ...
    public ELF(OpenTTY midlet, Hashtable args, Object stdout, Hashtable scope, int id, String pid, Process proc) { this.midlet = midlet; this.stdout = stdout; this.scope = scope; }
    ...
    public boolean load(byte[] elfData) throws Exception {
        ...
        if (elfData[4] != ELFCLASS32) { midlet.print("Only 32-bit ELF supported", stdout, 0, scope); return false; }
        if (elfData[5] != ELFDATA2LSB) { midlet.print("Only little-endian ELF supported", stdout, 0, scope); return false; }
        ...
        if (e_type != ET_EXEC) { midlet.print("Not an executable ELF", stdout, 0, scope); return false; }
        if (e_machine != EM_ARM) { midlet.print("Not an ARM executable", stdout, 0, scope); return false; }
    ...
    public Hashtable run() {
        ...
        midlet.print("ELF not supported!", stdout, 0, scope);
