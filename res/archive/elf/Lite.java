...
public class ELF {
    ...
    public boolean load(byte[] elfData) throws Exception {
        ...
        if (e_machine != EM_RISCV) { midlet.print("ELF invalid: not a RISC-V executable", stdout, id, scope); return false; }

        midlet.print("ELF valid, but not supported (build without RISC-V emulator)", stdout, id, scope);
        return true;
    }
    ...
    /* Kept for Process task-manager compatibility with the full emulator. */
    public void kill() { }
