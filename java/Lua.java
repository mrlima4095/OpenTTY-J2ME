...
public class Lua {
    ...
    // |
    // Static helpers (moved from inner class for Java 8 compatibility)
    ...
    public class LuaFunction implements Runnable, CommandListener, ItemCommandListener, ItemStateListener {
        ...
        public Object internals(Vector args) throws Exception {
            ...
            switch (MOD) {
                ...
                case VIBRATE: midlet.display.vibrate(args.isEmpty() ? new Integer(500) : args.elementAt(0) instanceof Double ? new Integer(((Double) args.elementAt(0)).intValue()) : (Integer) gotbad(1, "vibrate", "number expected")); break;
        ...
        String type(Object item) { return Lua.type(item); }
        ...
        // |
        boolean isListTable(Hashtable table) { return Lua.isListTable(table); }
        Vector toVector(Hashtable table) throws Exception { return Lua.toVector(table); }
        ...
        public Double exec(Vector args) throws Exception {
            if (args.isEmpty()) { return (Double) gotbad(1, "execute", "string expected, got no value"); }
                ...
                String[] argParts = midlet.splitArgs(argument);

                Object output = stdout; Hashtable aliases = (Hashtable) father.get("ALIAS");
                for (int i = 0; i < argParts.length; i++) {
                    if (argParts[i].equals(">")) {
                        output = toLuaString(midlet.joinpath(argParts[i + 1], father));
                        
                        Vector sanitize = new Vector(); StringBuffer buffer = new StringBuffer();
                        for (int j = 0; j < i - 1; j++) { sanitize.addElement(argParts[j]); buffer.append(argParts[j]); }

                        argParts = new String[sanitize.size()];
                        sanitize.copyInto(argParts); argument = buffer.toString();
                ...
                if (mainCommand.equals("") || mainCommand.equals("true") || mainCommand.startsWith("#")) { }
                    ...
                    else if ((in = open(midlet.joinpath(argParts[0], father), father)) != null) {
                        status = (Integer) popen(argParts[0], midlet.genpid(), argument.substring(argParts[0].length()).trim(), id, output, father, in).elementAt(0);
                    }
                    else {
                        midlet.print(". " + argParts[0] + ": not found", output, id, father);
                        status = 127;
                    }
                }
                    ...
                    for (int i = 0; i < argParts.length; i++) {
                        try {
                            InputStream in = midlet.getInputStream(midlet.joinpath(argParts[i], father), father);
                    ...
                    payload.addElement(argParts.length == 0 ? (String) father.get("PWD") : midlet.joinpath(argParts[0], father));
                    ...
                    if (argParts.length >= 2) {
                        if (argParts[0].equals("root") && midlet.passwd(argParts[1])) { id = 0; father.put("USER", "root"); }
                        else { midlet.print("Permission denied!", output, id, father); status = 13; }
                    } 
                    else if (argParts.length == 1) {
                        if (midlet.userID.containsKey(argParts[0])) {
                            id = midlet.getUserID(argParts[0]);
                            father.put("USER", argParts[0]);
                        } else {
                    ...
                    if (argParts.length == 0) {
                        ...
                        for (int i = 0; i < argParts.length; i++) {
                            int uid = midlet.getUserID(argParts[i]);

                            if (uid == -1) { midlet.print("id: " + argParts[i] + ": not found", output, id, father); status = 127; break; }
                            else { midlet.print("uid=" + uid + "(" + argParts[i] + ")", output, id, father); }
                        }
                    }
                }
                    ...
                    if (argParts.length == 0) {
                        ...
                        for (int i = 0; i < argParts.length; i++) {
                            int j = argParts[i].indexOf("="); 
                            if (j != -1) {
                                String key = argParts[i].substring(0, j), value = midlet.getpattern(argParts[i].substring(j + 1));
                                aliases.put(key, value);
                            } else {
                                if (aliases.containsKey(argParts[i])) {
                                    midlet.print("alias " + argParts[i] + "='" + ((String) aliases.get(argParts[i])) + "'", output, id, father);
                                } else {
                                    midlet.print("alias: " + argParts[i] + ": not found", output, id, father); status = 127;
                                }
                            }
                        }
                    }
                    ...
                    if (argParts.length == 0) { midlet.print("unalias: usage: unalias [-a] name [name ...]", output, id, father); }
                    else if (argParts[0].equals("-a")) { aliases.clear(); }
                    else {
                        for (int i = 0; i < argParts.length; i++) {
                            if (aliases.containsKey(argParts[i])) {
                                aliases.remove(argParts[i]);
                            }
                            else {
                                midlet.print("unalias: " + argParts[i] + ": not found", output, id, father);
                                status = 127;
                                break;
                            }
                        }
                    ...
                    if (argParts.length == 0) {
                        ...
                        for (int i = 0; i < argParts.length; i++) {
                            ...
                            if (j != -1) {
                                ...
                                if (midlet.attributes.containsKey(argParts[i])) {
                                    midlet.print(argParts[i] + "=" + ((String) midlet.attributes.get(argParts[i])), output, id, father);
                                } else {
                                    midlet.print(mainCommand + ": " + argParts[i] + ": not found", output, id, father); status = 127;
                                }
                            }
                        }
                    }
                    ...
                    if (argParts.length == 0) { }
                        ...
                        for (int i = 0; i < argParts.length; i++) {
                            if (midlet.attributes.containsKey(argParts[i])) {
                                midlet.attributes.remove(argParts[i]);
                            }
                            else {
                                midlet.print("unset: " + argParts[i] + ": not found", output, id, father);
                                status = 127;
                                break;
                            }
                        }
                ...
                else if (mainCommand.equals("exit")) { Vector payload = new Vector(); payload.addElement(argParts.length == 0 ? "0" : argParts[0]); payload.addElement(builtin); exit(payload); }
                else if (mainCommand.equals("pwd")) { midlet.print((String) father.get("PWD"), output, id, father); }
                else if (mainCommand.equals("cd")) {
                    Vector payload = new Vector();
                    payload.addElement(argParts.length == 0 ? "/home/" : argParts[0]);
                    status = ((Double) chdir(payload)).intValue();

                    if (status == 127) { midlet.print("cd: " + argParts[0] + ": not found", output, id, father); }
                    else if (status == 20) { midlet.print("cd: " + argParts[0] + ": found", output, id, father); }
                }
