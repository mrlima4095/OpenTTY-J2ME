...
- Guest C library via lightweight SYSCALL wrappers (`LIB_BASE=1000`): ids 1000–1054 in `ELF.java` `handleLibraryCall` cover string (`strlen…strdup`), `atoi`/`abs`/`toupper`/`tolower`, memory (`memcpy/memmove/memset/memcmp/memchr`), IO (`putchar/puts/printf/sprintf/snprintf`), heap (`malloc/calloc/realloc/free`), `getpid`, RISC-V runtime helpers, LCDUI (`res/lib/lcdui.h`), `graphics_taskmngr`, and ELF equivalents for `os.setproc("name", ...)`/screen registration. `exit/_exit/abort` = syscall 1. Raw Linux syscalls (`write/read/open/close/brk`) use the emulator's fixed EABI numbers: load into a7 and `ecall`.
