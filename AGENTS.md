...
- `res/` — embedded resources (lua modules under `res/lua/modules/`, bundled apps, pages). `res/lib/` holds the guest C runtime for the emulator: `lib32.s` (full asm stdlib) and `libc.s` (thin `svc #LIB_*; bx lr` wrappers) — **still ARM32, RISC-V port pending** (`res/lib/*.s` don't assemble with the RISC-V toolchain yet).
- `res/archive/elf/` — emulator archives for `res/swap_lite.sh`: `Full.java` (full RV32IM emulator, backed up here) and `Lite.java` (lite stub, marker `// ELF Lite`). `swap_lite.sh` swaps `src/ELF.java` between them; the legacy `res/archive/ELF.java`/`ELF.full.java` are stale ARM-era files.
- `build-elf.sh` — builds RISC-V RV32IM ELFs for the emulator (`ET_EXEC`, ELF32 LE, `EM_RISCV`=243, entry ≤ 1 MB). Flags: `-T <addr>`, `-entry <sym>`, `-shared`, `-keep`; `CROSS=` overrides the toolchain search (`riscv64-unknown-elf-`, `riscv64-linux-gnu-`, …). **`-lib`/`-stdlib` are blocked for now** (ARM asm, port pending). `.c` files compile with `-march=rv32im -mabi=ilp32 -fno-builtin`; `ld` links with `-m elf32lriscv`.
...
- Machine: `EM_RISCV`=243, 32-bit, 1 MB guest RAM, `registers[32]` (x0 hardwired 0; `REG_SP`=2 x2, `REG_LR`=1 x1/ra, `REG_R0..R3`=x10..13, `REG_R7`=x17/a7). Decoder: RV32I (LUI/AUIPC/JAL/JALR/BL* /LBHWLHU/SB/SH/SW/ALU/SLL*/SR* /FENCE/SYSTEM) + M-ext (MUL/MULH/MULHSU/MULHU/DIV/DIVU/REM/REMU). Syscall via `ecall` → `handleSyscall(registers[REG_R7])`; `ebreak` stops.
- Guest C library via lightweight SYSCALL wrappers (`LIB_BASE=1000`): ids 1000–1038 in `ELF.java` `handleLibraryCall` cover string (`strlen…strdup`), `atoi`/`abs`/`toupper`/`tolower`, memory (`memcpy/memmove/memset/memcmp/memchr`), IO (`putchar/puts/printf/sprintf/snprintf`), heap (`malloc/calloc/realloc/free`), `getpid` + AEABI helpers. `exit/_exit/abort` = syscall 1. Raw Linux syscalls (`write/read/open/close/brk`) use the same ARM-EABI numbers — the guest RISC-V libc (port pending in `res/lib/*.s`) must load them into a7 and `ecall`.
...
- Demo ELF: `/tmp/opencode/mkrv.py` builds a hand-RV32I `rvtest.elf` (write + exit); `TestRV.java` runs it against the stubs. Host-side `javac -d /tmp/jc -cp /tmp/opencode/tty-stubs src/ELF.java /tmp/opencode/tty-stubs/{Lua,OpenTTY,Process}.java` for typechecking. Dynamic linking (PLT/GOT resolver stubs) still emits ARM code — RISC-V port pending.
- Sample C apps: `res/apps/src/*.c` → `res/apps/dist/<name>`; the ones using `-stdlib` need the `res/lib` RISC-V port first.
