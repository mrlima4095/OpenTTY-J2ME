...
- Java exceptions (e.g. `java.lang.NullPointerException`) carry no message. The runtime appends a **Lua-side traceback** — `Lua <file>` + a `stack traceback:` of the Lua function chain — via `Lua.getTraceback(e)`, used in `run()`, `pcall`, `os.request` handler failures, background threads, and UI callbacks. This depends on `currentSource`/`frameStack`/`thrownFrames` on the Lua instance and `LuaFunction.name` — keep these in sync in both `src/` and `java/`.
